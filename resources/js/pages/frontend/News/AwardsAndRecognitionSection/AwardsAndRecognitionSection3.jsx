// frontend/Blog/AwardsAndRecognitionSection/AwardsAndRecognitionSection3.jsx

/**
 * Awards & Recognition Section - Awards Hub with Modal
 *
 * Unique design elements:
 * - Tabbed interface (All Awards, Hall of Fame, Yearly Awards, Saved)
 * - Featured awards carousel with auto-play and manual navigation
 * - Award detail modal with full information
 * - Hall of Fame section for prestigious awards
 * - Yearly awards tab for current year
 * - Saved awards tab with bookmark persistence
 ...
 * - Award timeline with year-based chronological display
...
const AwardsAndRecognitionSection3 = ({ config }) => {
  ...
  const awardTimeline = useMemo(() => config?.awardTimeline || [], [config?.awardTimeline]);
  const featuredAwards = useMemo(() => config?.featuredAwards || [], [config?.featuredAwards]);
  ...
  const nextSlide = useCallback(() => {
    setCurrentSlide((prev) => (prev + 1) % (featuredAwards.length || 1));
  }, [featuredAwards.length]);

  const prevSlide = useCallback(() => {
    setCurrentSlide((prev) => (prev - 1 + (featuredAwards.length || 1)) % (featuredAwards.length || 1));
  }, [featuredAwards.length]);
  ...
  // ==================== AUTO-PLAY CAROUSEL ====================
  useEffect(() => {
    if (config?.autoPlayCarousel && featuredAwards.length > 1) {
      const interval = setInterval(nextSlide, 6000);
      return () => clearInterval(interval);
    }
  }, [config?.autoPlayCarousel, featuredAwards.length, nextSlide]);
  ...
  const filteredAwards = useMemo(() => {
    ...
    return awards;
  }, [allAwards, searchQuery, selectedCategory, selectedYear, activeTab, savedAwards]);
            ...
            {stats.map((stat, idx) => (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center">
                  {getIcon(stat.icon, 'w-4 h-4 text-yellow-600 dark:text-yellow-400')}
                </div>
                  ...
                  return (
                    <div key={award.id} className="w-full shrink-0">
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && openAwardModal(award)}
                      >
                        <img
                          src={award.image}
                          alt={award.title}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 text-white">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              ...
                              aria-label={savedAwards.includes(award.id) ? 'Remove from saved' : 'Save award'}
                            >
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-900 dark:text-white placeholder-gray-500"
                  ...
                  {award.quote && (
                    <div className="mb-3">
                      <button
                        ...
                        className="flex items-center gap-1 text-xs text-yellow-600 dark:text-yellow-400 font-medium hover:gap-2 transition-all duration-300"
                      ...
                      {isExpanded && (
                        <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border-l-3 border-yellow-500 animate-fadeIn">
        ...
        {activeTab === 'all' && awardTimeline.length > 0 && (
          <div className="mt-12">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
              {getIcon('clock', 'w-5 h-5 text-yellow-600')}
              Award Timeline
            </h2>
                ...
                {awardTimeline.map((item, idx) => (
                  <div key={idx} className="relative pl-10">
                    <div className="absolute left-0 top-1 w-6 h-6 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center">
                      {getIcon('trophy', 'w-3 h-3 text-yellow-600')}
                    </div>
        ...
        {filteredAwards.length === 0 && (
          <div className="text-center py-16 bg-gray-50 dark:bg-gray-800/50 rounded-2xl mb-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {activeTab === 'saved'
                ? "You haven't saved any awards yet."
            ...
            {activeFiltersCount > 0 && (
              <button
                onClick={clearAllFilters}
                className="mt-4 text-yellow-600 dark:text-yellow-400 font-semibold hover:underline ml-4"
              >
                Clear all filters
              </button>
            )}
            ...
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4">
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${getCategoryConfig(selectedAward.category).color}`}
                    >
              ...
              <div className="p-6">
                ...
                {selectedAward.quote && (
                  <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border-l-4 border-yellow-500 mb-4">
                    {getIcon('quote', 'w-5 h-5 text-yellow-500 mb-2')}
                    <p className="text-sm italic text-gray-700 dark:text-gray-300">
                      "{selectedAward.quote.text}"
                    </p>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${likedAwards.includes(selectedAward.id)
                      ? 'text-red-500'
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${savedAwards.includes(selectedAward.id)
                      ? 'text-amber-500'
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors ml-auto"
                  >
                    View Award Page
