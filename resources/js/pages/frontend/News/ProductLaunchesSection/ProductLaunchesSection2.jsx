// page/frontend/News/ProductLaunchesSection/ProductLaunchesSection2.jsx
...
import { HiOutlineFire, HiOutlineRocketLaunch as HiOutlineRocket } from 'react-icons/hi2';
...
const ProductLaunchesSection2 = ({ config }) => {
    ...
    const getCategoryConfig = (category) => {
        const configs = {
            'ai': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'chip', label: 'AI & ML', borderColor: 'border-purple-200 dark:border-purple-800' },
            'automation': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'cog', label: 'Automation', borderColor: 'border-blue-200 dark:border-blue-800' },
            'analytics': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'chart', label: 'Analytics', borderColor: 'border-green-200 dark:border-green-800' },
            'integration': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'cloud', label: 'Integration', borderColor: 'border-orange-200 dark:border-orange-800' },
            'mobile': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'mobile', label: 'Mobile', borderColor: 'border-indigo-200 dark:border-indigo-800' },
    ...
    const getFilteredLaunches = useCallback(() => {
        ...
        if (sortBy === 'latest') {
            launches = [...launches].sort((a, b) => new Date(b.date) - new Date(a.date));
        } else if (sortBy === 'popular') {
            launches = [...launches].sort((a, b) => (b.views || 0) - (a.views || 0));
        } else if (sortBy === 'trending') {
            launches = [...launches].sort((a, b) => (b.trendingScore || 0) - (a.trendingScore || 0));
        }

        return launches;
    }, [config?.launches, searchQuery, selectedCategory, selectedStatus, sortBy]);

    const filteredLaunches = getFilteredLaunches();
    const categories = config?.categories || [
        { id: 'all', label: 'All Products', icon: 'rocket', count: config?.launches?.length || 0 },
    ...
    const popularLaunches = [...(config?.launches || [])]
    ...
    return (
        ...
        >
            {/* Background Pattern */}
            <div className="absolute inset-0 bg-gri d-slate-100 dark:bg-grid-slate-800 mask-[radial-gradient(ellipse_at_center,white,transparent)]" aria-hidden="true" />
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
                    <div>
                        <div className="inline-flex items-center gap-2 bg-purple-50 dark:bg-purple-900/30 rounded-full px-4 py-2 mb-4">
                            <HiOutlineRocket className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                        ...
                        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4">
                            {config?.title?.prefix || "Latest"} <span className="bg-linear-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">{config?.title?.highlight || "Product Releases"}</span>
                        </h1>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-8">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                            ...
                            <div className="relative p-8 md:p-10 text-white">
                                <div className="grid lg:grid-cols-2 gap-8 items-center">
                                    <div>
                                        ...
                                        <p className="text-white/90 mb-6 max-w-lg">{featuredLaunch.description}</p>
                                            ...
                                            {isExpanded && (
                                                <ul className="mt-3 space-y-2">
                                                    ...
                                                    {launch.features.length > 4 && (
                                                        <li className="text-sm text-gray-500 pl-6">+ {launch.features.length - 4} more features</li>
                                                    )}
                                                </ul>
                                            )}
                                        </div>
                    ...
                    <div className="mt-12">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {popularLaunches.map((launch) => (
                                <div key={launch.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                    <img src={launch.image} alt={launch.title} className="w-12 h-12 rounded-lg object-cover" />
                                    <div className="flex-1">
                                        <h4 className="font-medium text-gray-900 dark:text-white text-sm line-clamp-1">{launch.title}</h4>
                                        <p className="text-xs text-gray-500">{launch.views} views</p>
                                    </div>
                                    <Link href={launch.link} className="text-purple-600 text-sm font-semibold hover:underline">Learn</Link>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {config?.showNewsletter && (
                    <div className="mt-12 bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-200 dark:border-gray-700 text-center">
                        <HiOutlineBell className="w-12 h-12 mx-auto text-purple-600 dark:text-purple-400 mb-4" />
                        ...
                        <form className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto">
                            <input
                                ...
                                className="flex-1 px-6 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
