// page/frontend/News/ProductLaunchesSection/ProductLaunchesSection3.jsx
...
const ProductLaunchesSection3 = ({ config }) => {
    ...
    const getStatusConfig = (status) => {
        const configs = {
            'live': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'rocket', label: 'Now Live', badge: 'LIVE', gradient: 'from-green-500 to-emerald-500' },
            'beta': { color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', icon: 'chip', label: 'Beta', badge: 'BETA', gradient: 'from-yellow-500 to-orange-500' },
            'coming-soon': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'clock', label: 'Coming Soon', badge: 'SOON', gradient: 'from-blue-500 to-indigo-500' },
            'preview': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'eye', label: 'Preview', badge: 'PREVIEW', gradient: 'from-purple-500 to-pink-500' }
        };
    ...
    const getCategoryConfig = (category) => {
        const configs = {
            'ai': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'chip', label: 'AI & ML', gradient: 'from-purple-500 to-purple-600' },
            'automation': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'cog', label: 'Automation', gradient: 'from-blue-500 to-blue-600' },
            'analytics': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'chart', label: 'Analytics', gradient: 'from-green-500 to-green-600' },
            'integration': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'cloud', label: 'Integration', gradient: 'from-orange-500 to-orange-600' },
            'mobile': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'mobile', label: 'Mobile', gradient: 'from-indigo-500 to-indigo-600' },
            'security': { color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'shield', label: 'Security', gradient: 'from-red-500 to-red-600' }
        };
    ...
    // Handle like launch
    const handleLikeLaunch = (launchId) => {
    ...
    const nextSlide = useCallback(() => {
        setCurrentSlide((prev) => (prev + 1) % (config?.featuredLaunches?.length || 1));
    }, [config?.featuredLaunches?.length]);

    const prevSlide = () => {
        setCurrentSlide((prev) => (prev - 1 + (config?.featuredLaunches?.length || 1)) % (config?.featuredLaunches?.length || 1));
    };

    // Auto-play carousel
    useEffect(() => {
        if (config?.autoPlayCarousel && config?.featuredLaunches?.length > 1) {
            const interval = setInterval(() => {
                nextSlide();
            }, 6000);
            return () => clearInterval(interval);
        }
    }, [config?.autoPlayCarousel, config?.featuredLaunches?.length, nextSlide]);

    // Filter launches
    const getFilteredLaunches = useCallback(() => {
        ...
        return [...launches].sort((a, b) => new Date(b.date) - new Date(a.date));
    }, [config?.launches, searchQuery, selectedCategory, selectedStatus, activeTab, savedLaunches]);
    ...
    const featuredLaunches = config?.featuredLaunches || [];
    ...
    const activeFiltersCount = [selectedCategory !== 'all', selectedStatus !== 'all', searchQuery !== ''].filter(Boolean).length;
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-4xl mx-auto mb-12">
                    <div className="inline-flex items-center gap-2 bg-linear-to-r from-purple-600 to-blue-600 text-white px-4 py-2 rounded-full mb-6 shadow-lg animate-pulse">
                        <HiOutlineRocket className="w-4 h-4" />
                        <span className="text-sm font-medium">{config?.badge || "Launch Hub"}</span>
                    </div>

                    <h1 className="text-5xl md:text-7xl font-bold text-gray-900 dark:text-white mb-6">
                        {config?.title?.prefix || "Product"} <span className="bg-linear-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">{config?.title?.highlight || "Launches"}</span>
                    </h1>
                    ...
                    {tabs.map((tab) => (
                        ...
                        >
                            ...
                            {tab.id === 'saved' && savedLaunches.length > 0 && (
                                <span className="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-xs">{savedLaunches.length}</span>
                            )}
                        </button>
                    ))}
                </div>
                                        ...
                                        <div key={launch.id} className="w-full shrink-0">
                                            <div className="relative h-125 rounded-3xl overflow-hidden">
                                                ...
                                                <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                                                    ...
                                                    <h2 className="text-3xl md:text-4xl font-bold mb-3">{launch.title}</h2>
                                                    <p className="text-white/80 text-lg mb-4 max-w-2xl line-clamp-2">{launch.description}</p>
                ...
                <div className="mb-6">
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Showing <span className="font-semibold text-gray-900 dark:text-white">{filteredLaunches.length}</span> products
                            ...
                            >
                                ...
                                <div className="p-5">
                                    ...
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 line-clamp-2">
                                        <Link href={launch.link} className="hover:text-purple-600 dark:hover:text-purple-400">
                                            {launch.title}
                                        </Link>
                                    </h3>
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`text-xs transition-colors ${isSelectedForCompare ? 'text-purple-600' : 'text-gray-400 hover:text-purple-600'}`}
                                            >
                                                Compare
                                            </button>
                                        </div>
                                        <Link href={launch.link} className="text-purple-600 text-xs font-semibold hover:underline">
                                            Learn More →
                                        </Link>
                                    </div>
                                </div>
                ...
                {filteredLaunches.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            {activeTab === 'saved' ? "You haven't saved any products yet." : "Try adjusting your search or filter criteria"}
                        </p>
                        {activeTab === 'saved' && (
                ...
                {selectedForComparison.length > 0 && (
                    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-4">
                ...
                {/* Comparison Modal */}
                {showComparisonModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowComparisonModal(false)}>
                        <div className="relative max-w-5xl w-full max-h-85vh bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-gray-700">
                                            <th className="text-left py-3 font-semibold text-gray-700 dark:text-gray-300">Feature</th>
                                            {selectedForComparison.map(id => {
                                                const launch = config?.launches?.find(l => l.id === id);
                                                return (
                                                    <th key={id} className="text-left py-3 px-4">
                                                        <div className="flex items-center gap-2">
                                                            <img src={launch?.image} alt={launch?.title} className="w-8 h-8 rounded object-cover" />
                                                            <span className="font-semibold text-gray-900 dark:text-white">{launch?.title}</span>
                                                        </div>
                                                    </th>
                                                );
                                            })}
                                    ...
                                    <tbody>
                                            ...
                                            {selectedForComparison.map(id => {
                                                ...
                                                return (
                                                    <td key={id} className="py-3 px-4">
                                                        <span className={`px-2 py-1 rounded-full text-xs ${statusConfig.color}`}>{statusConfig.label}</span>
                                                    </td>
                                                );
                                            })}
                                        </tr>
                                        <tr className="border-b border-gray-100 dark:border-gray-700">
                                            ...
                                            {selectedForComparison.map(id => {
                                                const launch = config?.launches?.find(l => l.id === id);
                                                return <td key={id} className="py-3 px-4">{formatDate(launch?.date, 'short')}</td>;
                                            })}
                                        </tr>
                                        <tr className="border-b border-gray-100 dark:border-gray-700">
