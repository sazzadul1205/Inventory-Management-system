...
/**
 * User Groups Section II - Advanced Community Hub with Forums & Events
 *
 * Unique Design Elements:
 * - Stats Cards with Trend Indicators
 * - Multi-tab UI (All Groups, Featured, Near You, Online, My Groups)
 * - Group Discussion Forum with Post Creation
 * - Event Calendar with RSVP Functionality
 * - Community Leader Profiles with Verification Badges
 * - Resource Library for Group Materials
 * - Join Modal with Terms & Conditions
 * - Group Leader Showcase in Featured Banner
...
const UserGroupsSection2 = ({ config }) => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', company: '', role: '', experience: '', interests: '', newsletter: false, terms: false, });
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative mb-12 rounded-3xl overflow-hidden bg-linear-to-r from-blue-600 to-purple-600 shadow-xl">
            ...
            <div className="relative p-8 md:p-12 text-white">
              ...
              {featuredGroup.leader && (
                <div className="items-center gap-3 mb-6 p-3 bg-white/10 rounded-xl inline-flex">
                  {featuredGroup.leader.avatar ? (
                    <img src={featuredGroup.leader.avatar} alt={featuredGroup.leader.name} className="w-10 h-10 rounded-full object-cover" />
                  ) : (
                  ...
                  <button
                    onClick={() => { setSelectedLeader(featuredGroup.leader); setShowLeaderModal(true); }}
                    className="ml-2 text-xs bg-white/20 px-2 py-1 rounded-lg hover:bg-white/30 transition-colors"
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <button
                  onClick={() => { setForumGroup(featuredGroup); setShowForumModal(true); }}
                ...
                <button
                  onClick={() => { setResourceGroup(featuredGroup); setShowResourceModal(true); }}
                  className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-6 py-3 rounded-xl font-semibold hover:bg-white/30 transition-all duration-300"
        ...
        {displayedGroups.length === 0 ? (
          <div className="text-center py-12">
            ...
            <button
              onClick={() => { setSearchQuery(''); setSelectedRegion('all'); setSelectedTopic('all'); }}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {upcomingEvents.length > 0 && (
                        <button onClick={() => { setEventGroup(group); setShowEventModal(true); }} className="text-xs text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1" aria-label="View events">
                          <HiOutlineCalendar className="w-3 h-3" />{upcomingEvents.length} upcoming
                        </button>
                      )}
                      {groupPosts.length > 0 && (
                        <button onClick={() => { setForumGroup(group); setShowForumModal(true); }} className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline flex items-center gap-1" aria-label="View discussions">
                          <HiOutlineChat className="w-3 h-3" />{groupPosts.length} discussions
                        </button>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {isJoined ? (
                        <>
                          <button onClick={() => { setForumGroup(group); setShowForumModal(true); }} className="flex-1 inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl font-semibold text-sm transition-colors" aria-label="View discussions">
                            <HiOutlineChat className="w-4 h-4" />Discussions
                          </button>
                          <button onClick={(e) => leaveGroup(group.id, e)} className="inline-flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-700 text-red-600 dark:text-red-400 px-4 py-2 rounded-xl font-semibold text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors" aria-label="Leave group">
                            Leave
                          </button>
                        </>
                      ) : (
                        <button onClick={() => { setSelectedGroup(group); setShowJoinModal(true); }} className="flex-1 inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-semibold text-sm transition-colors" aria-label="Join group">
                          <HiOutlineUserGroup className="w-4 h-4" />Join Group
                        </button>
                      )}
                      <button onClick={() => setExpandedGroupId(isExpanded ? null : group.id)} className="text-blue-600 dark:text-blue-400 hover:underline text-sm font-semibold" aria-label={isExpanded ? "Show less details" : "Show more details"}>
                    ...
                    {isExpanded && (
                      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
                        {group.leader && (
                          <div className="mb-3">
                            <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Group Leader:</p>
                            <div className="flex items-center gap-2">
                              <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                                <HiOutlineUser className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                              </div>
                              <div>
                                <p className="text-sm font-medium text-gray-900 dark:text-white">{group.leader.name}</p>
              ...
              <div className="p-6">
                {formSubmitted ? (
                  <div className="text-center py-8 animate-fadeIn">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm">You've successfully joined {selectedGroup.name}. Check your email for group updates.</p>
                  </div>
                ) : (
                  <form onSubmit={handleJoinGroup} className="space-y-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="newsletter" checked={formData.newsletter} onChange={handleInputChange} className="w-4 h-4" aria-label="Subscribe to newsletter" /><span className="text-sm text-gray-600 dark:text-gray-400">Receive email updates about group events</span></label>
                    <label className={`flex items-start gap-2 cursor-pointer ${errors.terms ? 'text-red-500' : ''}`}><input type="checkbox" name="terms" checked={formData.terms} onChange={handleInputChange} className="w-4 h-4 mt-0.5" aria-label="Agree to community guidelines" /><span className="text-sm text-gray-600 dark:text-gray-400">I agree to the <a href="#" className="text-blue-600 hover:underline">Community Guidelines</a> *</span></label>
                    {errors.terms && <p className="text-red-500 text-xs">{errors.terms}</p>}
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300" aria-label="Submit join request">Join Group<HiOutlineArrowRight className="inline ml-2 w-4 h-4" /></button>
                  </form>
                )}
              </div>
            </div>
        ...
        {showForumModal && forumGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowForumModal(false)} role="dialog" aria-label="Group Discussions" aria-modal="true">
        ...
        {showEventModal && eventGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowEventModal(false)} role="dialog" aria-label="Upcoming Events" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-purple-600 p-4 flex items-center justify-between">
                <h3 className="text-white font-bold text-lg">Upcoming Events - {eventGroup.name}</h3>
                <button onClick={() => setShowEventModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal"><HiOutlineX className="w-6 h-6" /></button>
              </div>
              <div className="p-6 max-h-96 overflow-y-auto">
                      ...
                      return (
                        <div key={event.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
                          ...
                          <div className="flex gap-2 mt-3">
                            {status === 'going' ? (<span className="text-xs text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-1 rounded-full">Going ✓</span>) : (<button onClick={() => handleRSVP(eventGroup.id, event.id, 'going')} className="text-xs bg-blue-600 text-white px-3 py-1 rounded-lg hover:bg-blue-700 transition-colors">RSVP: Going</button>)}
                            {status === 'interested' ? (<span className="text-xs text-amber-600 bg-amber-100 dark:bg-amber-900/30 dark:text-amber-400 px-2 py-1 rounded-full">Interested</span>) : (<button onClick={() => handleRSVP(eventGroup.id, event.id, 'interested')} className="text-xs bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-300 px-3 py-1 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Interested</button>)}
                          </div>
                        </div>
                      );
                    })}
                  </div>
                ) : (<div className="text-center py-8"><HiOutlineCalendar className="w-12 h-12 mx-auto text-gray-300 dark:text-gray-600 mb-3" /><p className="text-gray-500 dark:text-gray-400">No upcoming events scheduled.</p></div>)}
              </div>
            </div>
          </div>
        )}
        ...
        {showLeaderModal && selectedLeader && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowLeaderModal(false)} role="dialog" aria-label="Community Leader Profile" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-blue-600 p-4 flex items-center justify-between">
                <h3 className="text-white font-bold text-lg">Community Leader</h3>
                <button onClick={() => setShowLeaderModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal"><HiOutlineX className="w-6 h-6" /></button>
              </div>
              <div className="p-6 text-center">
                {selectedLeader.avatar ? (<img src={selectedLeader.avatar} className="w-24 h-24 rounded-full object-cover mx-auto mb-4" />) : (<div className="w-24 h-24 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center mx-auto mb-4"><HiOutlineUser className="w-12 h-12 text-blue-600 dark:text-blue-400" /></div>)}
                ...
                {selectedLeader.verified && (<div className="inline-flex items-center gap-1 text-xs text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-1 rounded-full mb-3"><HiOutlineBadgeCheck className="w-3 h-3" />Verified Leader</div>)}
                <p className="text-gray-600 dark:text-gray-400 text-sm">{selectedLeader.bio}</p>
                <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg text-left"><p className="text-sm font-semibold">Expertise:</p><div className="flex flex-wrap gap-1 mt-1">{selectedLeader.expertise?.map((exp, idx) => (<span key={idx} className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full">{exp}</span>))}</div></div>
                <button className="mt-4 w-full py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors">Connect with Leader</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showResourceModal && resourceGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowResourceModal(false)} role="dialog" aria-label="Group Resources" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-orange-600 p-4 flex items-center justify-between">
                <h3 className="text-white font-bold text-lg">Resources - {resourceGroup.name}</h3>
                <button onClick={() => setShowResourceModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal"><HiOutlineX className="w-6 h-6" /></button>
              </div>
              <div className="p-6 max-h-96 overflow-y-auto">
                  ...
                  <div className="space-y-3">
                    {resourceGroup.resources.map((resource, idx) => (
                      <div key={idx} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div className="flex items-center justify-between"><div><h4 className="font-medium text-gray-900 dark:text-white text-sm">{resource.title}</h4><p className="text-xs text-gray-500 mt-1">{resource.type} • {resource.size}</p></div><button className="text-blue-600 dark:text-blue-400 hover:underline text-sm">Download</button></div>
                      </div>
                    ))}
                  </div>
                ) : (<div className="text-center py-8"><HiOutlineClipboardList className="w-12 h-12 mx-auto text-gray-300 dark:text-gray-600 mb-3" /><p className="text-gray-500 dark:text-gray-400">No resources available yet.</p></div>)}
              </div>
            </div>
          </div>
        )}
        ...
        {showShareModal && shareGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowShareModal(false)} role="dialog" aria-label="Share User Group" aria-modal="true">
        ...
        <div className="mt-12 bg-linear-to-r from-blue-600 to-purple-600 dark:from-blue-500 dark:to-purple-500 rounded-3xl p-8 text-white text-center">
          ...
          <button className="inline-flex items-center gap-2 bg-white text-blue-600 px-8 py-4 rounded-xl font-semibold hover:bg-blue-50 transition-all duration-300 transform hover:scale-105 shadow-lg" aria-label="Start a new group">
            <HiOutlineUserGroup className="w-5 h-5" />Start a New Group<HiOutlineArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
