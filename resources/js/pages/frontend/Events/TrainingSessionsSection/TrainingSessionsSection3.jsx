...
/**
 * Training Sessions Section III - Premium Learning Hub with Live Classes & Community
 *
 * Unique Design Elements:
 * - Stats Cards for Training Metrics (Courses, Students, Live Classes, Certificates)
 * - Featured Course Banner with Live Class Indicator
 * - Course Player with Module Navigation and Progress Tracking
 * - Live Class Modal with Chat, Hand Raise, and Attendance Features
 * - Discussion Forum with Post Creation and Likes
 ...
 * - Live Tab for Real-time Class Sessions
 * - Animated Pulse Badge for Live Classes
...
const TrainingSessionsSection3 = ({ config }) => {
  ...
  const getCourseAnalytics = (sessionId) => {
    const progress = courseProgress[sessionId];
    const session = trainingSessions.find(s => s.id === sessionId);
    const sessionQuizResults = quizResults;

    const avgQuizScore = Object.entries(sessionQuizResults)
      .filter(([key]) => key.startsWith(sessionId))
      .reduce((acc, [, value]) => acc + value.score, 0) / (Object.keys(sessionQuizResults).filter(key => key.startsWith(sessionId)).length || 1);

    return {
      progress: progress?.overallProgress || 0,
      avgQuizScore: Math.round(avgQuizScore),
      completedModules: progress?.completedModules?.length || 0,
      totalModules: session?.modules?.length || 0,
      quizzesTaken: Object.keys(sessionQuizResults).filter(key => key.startsWith(sessionId)).length,
      liveClassesAttended: Object.keys(liveClassAttendance).filter(key => key.startsWith(sessionId)).length,
    };
  };
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-5 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
                  <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                    {stat.icon === 'users' ? <HiOutlineUsers className="w-4 h-4 text-blue-600 dark:text-blue-400" /> :
                      stat.icon === 'academic' ? <HiOutlineAcademicCap className="w-4 h-4 text-blue-600 dark:text-blue-400" /> :
          ...
          <div className="relative mb-12 rounded-3xl overflow-hidden bg-linear-to-r from-blue-600 to-purple-600 shadow-xl">
            ...
            <div className="relative p-8 md:p-12 text-white">
              <div className="flex items-center gap-2 mb-4">
                ...
                {featuredSession.hasLiveClasses && (
                  <span className="ml-2 text-xs bg-white/20 px-2 py-1 rounded-full">Live Classes</span>
                )}
                {featuredSession.hasCertification && (
                  <span className="ml-2 text-xs bg-white/20 px-2 py-1 rounded-full">Certificate</span>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-6 mb-6 text-sm">
                ...
                {featuredSession.liveClasses && (
                  <div className="flex items-center gap-2"><HiOutlineVideoCamera className="w-4 h-4" /><span>{featuredSession.liveClasses.length} live sessions</span></div>
                )}
              </div>
                ...
                <div key={session.id} className="group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-200 dark:border-gray-700">
                  {session.image && (
                    <div className="relative h-48 overflow-hidden">
                      ...
                      {session.hasLiveClasses && (
                        <span className="absolute bottom-4 left-4 text-xs bg-red-500 text-white px-2 py-1 rounded-full animate-pulse">
                          Live
                        </span>
                      )}
                      {session.hasCertification && (
                        <span className="absolute bottom-4 right-4 text-xs bg-yellow-500 text-white px-2 py-1 rounded-full">
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-2 mb-4 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      {session.hasLiveClasses && (
                        <div className="flex items-center gap-2"><HiOutlineVideoCamera className="w-4 h-4 shrink-0 text-red-500" /><span>Live classes</span></div>
                      )}
                    </div>
                    <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {enrolled ? (
                        <>
                          ...
                          <button
                            onClick={() => { setAnalyticsCourse(session); setShowAnalyticsModal(true); }}
                            className="inline-flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-xl text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90" onClick={() => setShowCoursePlayer(false)} role="dialog" aria-label="Course Player" aria-modal="true">
            <div className="relative max-w-5xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl h-[90vh]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex flex-col md:flex-row h-full">
                      ...
                      {currentCourse.modules[currentModule].videoUrl && (
                        <div className="aspect-video bg-gray-900 rounded-xl mb-4 flex items-center justify-center">
                          <div className="text-center text-gray-400">
                            ...
                            <p>Video lesson would play here</p>
                          </div>
                        </div>
                      )}
                ...
                <div className="w-80 bg-gray-50 dark:bg-gray-700/50 p-4 border-l border-gray-200 dark:border-gray-700 overflow-y-auto">
                  <h5 className="font-semibold text-gray-900 dark:text-white mb-3">Course Content</h5>
                  ...
                  {currentCourse.hasLiveClasses && (
                    <div className="mb-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                      <h5 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
                        <HiOutlineVideoCamera className="w-4 h-4 text-red-500" />Live Classes
                      </h5>
                      <button
                        onClick={() => { setLiveClassCourse(currentCourse); setShowLiveClassModal(true); }}
                  ...
                  <div className="mb-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <h5 className="font-semibold text-gray-900 dark:text-white mb-3">Activities</h5>
                    <div className="space-y-2">
                      {currentCourse.quizzes && currentCourse.quizzes.length > 0 && (
                        <button
                          onClick={() => { setQuizCourse(currentCourse); setShowQuizModal(true); }}
                          className="w-full text-left p-2 rounded-lg text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-2 text-gray-700 dark:text-gray-300"
                          aria-label="Take quiz"
                        >
                          <HiOutlineQuestionMarkCircle className="w-4 h-4" />Take Quiz
                        </button>
                      )}
                      {currentCourse.hasAssignment && (
                        ...
                        >
                          <HiOutlineClipboardList className="w-4 h-4" />Submit Assignment
                        </button>
                      )}
                      {currentCourse.hasPeerReview && (
                        <button
                          onClick={() => { setPeerReviewCourse(currentCourse); setShowPeerReviewModal(true); }}
                          className="w-full text-left p-2 rounded-lg text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-2 text-gray-700 dark:text-gray-300"
                          aria-label="Peer reviews"
                        >
                          <HiOutlineUsers className="w-4 h-4" />Peer Reviews
                        </button>
                      )}
                      <button
                        onClick={() => { setDiscussionCourse(currentCourse); setShowDiscussionModal(true); }}
                        className="w-full text-left p-2 rounded-lg text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-2 text-gray-700 dark:text-gray-300"
                        aria-label="Discussion forum"
                      >
                        <HiOutlineChatAlt2 className="w-4 h-4" />Discussion Forum
                      </button>
                    </div>
                  </div>
                </div>
        ...
        {showLiveClassModal && liveClassCourse && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90" onClick={() => setShowLiveClassModal(false)} role="dialog" aria-label="Live Class" aria-modal="true">
            <div className="relative max-w-5xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl h-[85vh]" onClick={(e) => e.stopPropagation()}>
              <div className="bg-linear-to-r from-red-600 to-orange-600 p-4 flex items-center justify-between">
                <div>
                  <h3 className="text-white font-bold text-lg">Live Class: {liveClassCourse.title}</h3>
                  <p className="text-red-100 text-xs">Live Now</p>
                </div>
                <button onClick={() => setShowLiveClassModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close live class">
                  <HiOutlineX className="w-6 h-6" />
                </button>
              </div>
              <div className="flex h-full">
                <div className="flex-1 p-4">
                  <div className="aspect-video bg-gray-900 rounded-xl flex items-center justify-center mb-4">
                    <div className="text-center text-gray-400">
                      ...
                      <p className="text-sm mt-2">Instructor speaking • 24 attendees</p>
                    </div>
                  </div>
                ...
                <div className="w-80 bg-gray-50 dark:bg-gray-700/50 p-4 border-l border-gray-200 dark:border-gray-700 flex flex-col">
                  <h5 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
                    <HiOutlineChat className="w-4 h-4" />Live Chat
                  </h5>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                      onKeyPress={(e) => e.key === 'Enter' && handleSendChatMessage(liveClassCourse.id)}
                      aria-label="Chat message"
                    />
                    <button
                      onClick={() => handleSendChatMessage(liveClassCourse.id)}
                  ...
                  <button
                    onClick={() => handleLiveClassAttendance(liveClassCourse.id, 'live1')}
                    className="mt-3 w-full py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 transition-colors"
        ...
        {showDiscussionModal && discussionCourse && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowDiscussionModal(false)} role="dialog" aria-label="Discussion Forum" aria-modal="true">
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl h-[80vh]" onClick={(e) => e.stopPropagation()}>
              <div className="bg-indigo-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">Discussion Forum - {discussionCourse.title}</h3>
                  <button onClick={() => setShowDiscussionModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close forum">
                    <HiOutlineX className="w-6 h-6" />
                  </button>
                </div>
              </div>
              <div className="p-6 overflow-y-auto h-full">
                <div className="mb-4">
                  <textarea
                    ...
                    placeholder="Start a new discussion... (share insights, ask questions, or share resources)"
                    rows="3"
                    className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"
                    aria-label="New discussion post"
                  />
                  <div className="flex justify-end mt-2">
                    <button
                      onClick={() => handleDiscussionPost(discussionCourse.id)}
                    ...
                    <div key={post.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(post.timestamp).toLocaleDateString()}</p>
                        </div>
                        <button
                          onClick={() => likeDiscussionPost(discussionCourse.id, post.id)}
                          className="flex items-center gap-1 text-sm text-gray-500 hover:text-red-500 transition-colors"
                          aria-label="Like post"
                        >
                          <HiOutlineHeart className="w-4 h-4" />{post.likes}
                        </button>
                      </div>
        ...
        {showAnalyticsModal && analyticsCourse && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowAnalyticsModal(false)} role="dialog" aria-label="Learning Analytics" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-teal-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">Learning Analytics - {analyticsCourse.title}</h3>
                    ...
                    <div className="space-y-4">
                      <div className="text-center">
                        <div className="relative w-32 h-32 mx-auto">
                          <div className="w-32 h-32 rounded-full border-8 border-gray-200 dark:border-gray-700 flex items-center justify-center">
                            <span className="text-3xl font-bold text-teal-600 dark:text-teal-400">{analytics.progress}%</span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                          <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{analytics.completedModules}/{analytics.totalModules}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Modules Completed</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                          <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{analytics.avgQuizScore}%</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Avg Quiz Score</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                          <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{analytics.quizzesTaken}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Quizzes Taken</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                          <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{analytics.liveClassesAttended}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Live Classes</p>
                        </div>
                      </div>
                      {isCourseCompleted(analyticsCourse.id) && analyticsCourse.hasCertification && (
                        <button
                          onClick={() => { setCertificateSession(analyticsCourse); setShowCertificateModal(true); }}
                          className="w-full mt-4 py-3 bg-yellow-500 text-white rounded-xl font-semibold hover:bg-yellow-600 transition-colors"
            ...
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6">
                <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2"><strong>Instructions:</strong> Write a 500-word analysis applying course concepts to a real-world scenario.</p>
                </div>
                <textarea
        ...
        {showPeerReviewModal && peerReviewCourse && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowPeerReviewModal(false)} role="dialog" aria-label="Peer Review" aria-modal="true">
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-teal-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">Peer Review: {peerReviewCourse.title}</h3>
                  <button onClick={() => setShowPeerReviewModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal">
                    <HiOutlineX className="w-6 h-6" />
                  </button>
                </div>
              </div>
              <div className="p-6">
                <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-sm text-gray-600 dark:text-gray-400">Review a fellow student's assignment and provide constructive feedback.</p>
                </div>
                <textarea
                  value={peerReviewSubmission}
                  onChange={(e) => setPeerReviewSubmission(e.target.value)}
                  rows={6}
                  className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"
                  placeholder="Write your peer review here..."
                  aria-label="Peer review text"
                />
                <button
                  onClick={() => handlePeerReview(peerReviewCourse.id, 'assignment1', peerReviewSubmission)}
                  className="w-full mt-4 py-3 bg-teal-600 text-white rounded-xl font-semibold hover:bg-teal-700 transition-colors"
              ...
              <div className="p-6">
                {formSubmitted ? (
                  <div className="text-center py-8 animate-fadeIn">
                    ...
                    <button onClick={() => { setShowCoursePlayer(true); setShowEnrollModal(false); setCurrentCourse(selectedSession); }} className="mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" aria-label="Start learning">
                      Start Learning →
                    </button>
                  </div>
                ) : (
                  <form onSubmit={handleEnroll} className="space-y-4">
                    <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                      ...
                      {selectedSession.hasLiveClasses && (
                        <p className="text-xs text-red-500 dark:text-red-400 mt-1">Includes live classes</p>
                      )}
                    </div>
                    <div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowSyllabusModal(false)} role="dialog" aria-label="Course Syllabus" aria-modal="true">
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 max-h-96 overflow-y-auto">
                ...
                {syllabusSession.liveClasses && syllabusSession.liveClasses.length > 0 && (
                  <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <h4 className="font-semibold text-gray-900 dark:text-white mb-3">Live Sessions Schedule:</h4>
                    <div className="space-y-2">
                      {syllabusSession.liveClasses.map((lc, idx) => (
                        <div key={idx} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                          <HiOutlineVideoCamera className="w-4 h-4 text-red-500" />
                          <span>{lc.date}: {lc.topic}</span>
                        </div>
                      ))}
                    </div>
                  </div>
