...
/**
 * Event Calendar Section III - Enterprise Event Management with AI & Analytics
 *
 * Unique Design Elements:
 * - Stats Cards for Event Metrics (Total Events, Upcoming, Attendees, Waitlist)
 * - AI-Powered Scheduling Assistant Widget (Floating Button)
 * - Event Analytics Dashboard with Charts and Trends
 * - Priority-based Event Color Coding
 * - Multi-Calendar Support (Personal, Team, Company)
 ...
 * - Advanced Filtering (Category, Type, Priority)
 * - Multi-format Export (iCal, CSV, JSON)
 * - Event Status Tracking (Published, Draft, Cancelled)
...
const EventCalendarSection3 = ({ config }) => {
  ...
  const generateAISuggestions = useCallback(() => {
    ...
    suggestions.push({ type: 'resource', message: 'Main Conference Room is underutilized (only 2 events in the past month). Consider using it for workshops.' });
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Event Calendar Premium Hub"
    >
      {/* ==================== BACKGROUND PATTERN ==================== */}
      <div className="absolute inset-0 opacity-5 dark:opacity-10" aria-hidden="true">
        <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="circuit-pattern-ec" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div>
            <div className="inline-flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 rounded-full px-4 py-2 mb-4">
              ...
              <span className="text-sm font-medium text-blue-700 dark:text-blue-300">
                {config?.badge || "Enterprise Calendar"}
              </span>
            </div>
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
              {config?.description || "AI-powered scheduling, resource management, waitlist automation, and advanced analytics for enterprise event planning."}
            </p>
          </div>
          {stats.length > 0 && (
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowAnalyticsModal(true)} className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300 text-sm font-medium flex items-center gap-2" aria-label="Analytics">
              <HiOutlineChartBar className="w-4 h-4" />Analytics
            </button>
            <button onClick={() => setShowExportModal(true)} className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300 text-sm font-medium flex items-center gap-2" aria-label="Export">
              <HiOutlineDownload className="w-4 h-4" />Export
            </button>
            <button onClick={() => { setIsEditing(false); setShowEventModal(true); setFormData({ id: null, title: '', description: '', startDate: '', endDate: '', time: '', endTime: '', timezone: 'America/New_York', category: '', type: '', priority: 'medium', color: '#3B82F6', location: { venue: '', city: '', country: '', virtualLink: '' }, speaker: { name: '', title: '', company: '', email: '', bio: '' }, resources: [], recurring: { isRecurring: false, frequency: 'weekly', interval: 1, endDate: '', daysOfWeek: [] }, reminder: { enabled: true, minutes: 60, type: 'email' }, capacity: { maxAttendees: '', currentWaitList: 0 }, price: { amount: '', currency: 'USD', earlyBirdPrice: '', earlyBirdDeadline: '' }, registrationLink: '', tags: [], status: 'published' }); }} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-all duration-300 flex items-center gap-2" aria-label="Add event">
        ...
        <div className="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-700">
          {calendarViews.map(cal => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium transition-all duration-300 ${activeCalendar === cal ? 'border-b-2 border-blue-600 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
              aria-label={`Switch to ${cal} calendar`}
            >
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Filter by event type">
                    {eventTypes.map(t => <option key={t} value={t}>{t === 'all' ? 'All Types' : t === 'virtual' ? 'Virtual' : t === 'in-person' ? 'In-Person' : 'Hybrid'}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Priority</label>
                  <select value={selectedPriority} onChange={(e) => setSelectedPriority(e.target.value)} className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Filter by priority">
        ...
        {currentView === 'month' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white text-center">{getMonthName(currentDate)}</h3>
            </div>
            <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
              {daysOfWeek.map(day => (
                <div key={day} className="p-3 text-center text-sm font-semibold text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800">{day}</div>
              ))}
              {getDaysInMonth(currentDate).map(({ date, isCurrentMonth }, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`min-h-32 p-2 bg-white dark:bg-gray-800 transition-all duration-300 ${!isCurrentMonth ? 'opacity-40' : ''} ${dropTarget?.toDateString() === date.toDateString() ? 'bg-blue-50 dark:bg-blue-900/20 ring-2 ring-blue-500' : ''}`}
                  >
                    ...
                    <div className="mt-1 space-y-1">
                      {dayEvents.slice(0, 2).map((event, eventIdx) => (
                        <div
                          ...
                          role="button"
                          tabIndex={0}
                          onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && editEvent(event)}
                        >
                          {event.title}
                        </div>
                      ))}
                      {dayEvents.length > 2 && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 text-center">+{dayEvents.length - 2} more</div>
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="grid grid-cols-7 min-w-96">
              {getWeekEvents().map(({ date, events }, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="min-h-96 bg-white dark:bg-gray-800"
                    onDragOver={(e) => handleDragOver(e, date)}
                    onDrop={(e) => handleDrop(e, date)}
                  >
                    ...
                    <div className="p-2 space-y-2">
                      {events.map((event, eventIdx) => (
                        <div
                          ...
                          onDragStart={(e) => handleDragStart(event, e)}
                          onClick={() => editEvent(event)}
                          className="p-2 rounded-lg cursor-move hover:opacity-80 transition-all"
                        ...
                        >
                          ...
                          {event.time && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{formatTime(event.time)}</p>}
                        </div>
                      ))}
                ...
                getEventsForDate(currentDate).map((event, idx) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <button className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">Edit</button>
                    </div>
                  </div>
                ))
              ) : (
        ...
        {currentView === 'list' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">All Events</h3>
            </div>
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${event.priority === 'high' ? 'bg-red-100 text-red-700' : event.priority === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
            ...
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} ref={modalRef}>
              <div className="bg-linear-to-r from-blue-600 to-purple-600 p-4 sticky top-0">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">{isEditing ? 'Edit Event' : 'Create New Event'}</h3>
                  <button onClick={() => setShowEventModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal">
              ...
              <div className="p-6">
                {formSubmitted ? (
                  <div className="text-center py-8 animate-fadeIn">
                    ...
                    <h4 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{isEditing ? 'Event Updated!' : 'Event Created!'}</h4>
                  </div>
                ) : (
                  <form onSubmit={handleSaveEvent} className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Event Title *" className={`w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white placeholder-gray-500 ${errors.title ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}`} aria-label="Event title" />
                      </div>
                      <div>
                        <select name="priority" value={formData.priority} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Priority">
                    ...
                    <div>
                      <textarea name="description" value={formData.description} onChange={handleInputChange} placeholder="Description" rows="3" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white placeholder-gray-500 resize-none" aria-label="Event description" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                      ...
                      <div>
                        ...
                        <input type="date" name="endDate" value={formData.endDate} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="End date" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start Time</label>
                        <input type="time" name="time" value={formData.time} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Start time" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Time</label>
                        <input type="time" name="endTime" value={formData.endTime} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="End time" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
                        <select name="category" value={formData.category} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Select category">
                      ...
                      <div>
                        ...
                        <select name="type" value={formData.type} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Select event type">
                          <option value="">Select</option>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Event Color</label>
                        <input type="color" name="color" value={formData.color} onChange={handleInputChange} className="w-full h-12 px-3 py-1 border border-gray-200 dark:border-gray-600 rounded-xl" aria-label="Event color" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Timezone</label>
                        <select name="timezone" value={formData.timezone} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Timezone">
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="recurring.isRecurring" checked={formData.recurring.isRecurring} onChange={handleInputChange} className="w-4 h-4" aria-label="Recurring event" />
                      ...
                      {formData.recurring.isRecurring && (
                        <div className="grid grid-cols-2 gap-3 mt-3">
                          <select name="recurring.frequency" value={formData.recurring.frequency} onChange={handleInputChange} className="px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Recurring frequency">
                            <option value="daily">Daily</option>
                            <option value="weekly">Weekly</option>
                            <option value="monthly">Monthly</option>
                          </select>
                          <input type="number" name="recurring.interval" value={formData.recurring.interval} onChange={handleInputChange} className="px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white" aria-label="Recurring interval" placeholder="Interval" />
                        </div>
                      )}
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="number" name="capacity.maxAttendees" value={formData.capacity.maxAttendees} onChange={handleInputChange} placeholder="Unlimited" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white placeholder-gray-500" aria-label="Max attendees" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price</label>
                        <input type="text" name="price.amount" value={formData.price.amount} onChange={handleInputChange} placeholder="Free" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white placeholder-gray-500" aria-label="Price" />
                      </div>
                    </div>
                    <div className="flex gap-3 pt-4">
                      <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300">
                        {isEditing ? 'Update Event' : 'Create Event'}
                      </button>
                      {isEditing && (
                        <button type="button" onClick={() => deleteEvent(formData.id)} className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold transition-all duration-300">
                          Delete
                        </button>
                      )}
                    </div>
        ...
        {/* ==================== ANALYTICS MODAL ==================== */}
        {showAnalyticsModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowAnalyticsModal(false)} role="dialog" aria-label="Event Analytics" aria-modal="true">
            <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-teal-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">Event Analytics Dashboard</h3>
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/30 rounded-xl text-center">
                    <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{analyticsData.totalEvents || 0}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Total Events</p>
                  </div>
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl text-center">
                    <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{analyticsData.upcomingEvents || 0}</p>
                  ...
                  <div className="p-4 bg-purple-50 dark:bg-purple-900/30 rounded-xl text-center">
                    <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{analyticsData.totalAttendees || 0}</p>
                  ...
                  <div className="p-4 bg-orange-50 dark:bg-orange-900/30 rounded-xl text-center">
                    <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">{analyticsData.averageAttendance || 0}</p>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {Object.entries(analyticsData.popularCategories || {}).map(([cat, count]) => (
                      <div key={cat}>
                        ...
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                          <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${(count / (analyticsData.totalEvents || 1)) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(analyticsData.monthlyTrend || {}).slice(-6).map(([month, count]) => (
                      <div key={month}>
                        <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                          <span>{month}</span>
                          <span>{count} events</span>
                        ...
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                          <div className="bg-emerald-600 h-2 rounded-full" style={{ width: `${(count / Math.max(...Object.values(analyticsData.monthlyTrend || { 1: 1 }))) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {showAiModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowAiModal(false)} role="dialog" aria-label="AI Assistant" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-linear-to-r from-purple-600 to-pink-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg flex items-center gap-2">
                    <AiOutlineRobot className="w-5 h-5" /> AI Scheduling Assistant
                  </h3>
                  <button onClick={() => setShowAiModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal">
                    <HiOutlineX className="w-6 h-6" />
                  </button>
                </div>
              </div>
              <div className="p-6">
                {aiSuggestions.map((suggestion, idx) => (
                  <div key={idx} className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg border-l-4 border-purple-500">
                    <p className="text-sm text-gray-700 dark:text-gray-300">{suggestion.message}</p>
                  </div>
                ))}
                <button onClick={generateAISuggestions} className="w-full py-2 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700 transition-colors">Refresh Suggestions</button>
              </div>
            </div>
          </div>
        )}
        ...
        {/* ==================== EXPORT MODAL ==================== */}
        {showExportModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowExportModal(false)} role="dialog" aria-label="Export Calendar" aria-modal="true">
            <div className="relative max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="bg-emerald-600 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">Export Calendar</h3>
                  <button onClick={() => setShowExportModal(false)} className="text-white hover:text-gray-200 transition-colors" aria-label="Close modal">
                    <HiOutlineX className="w-6 h-6" />
                  </button>
                </div>
              </div>
              <div className="p-6">
                <div className="space-y-3">
                  <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                    <input type="radio" name="format" value="ica" checked={exportFormat === 'ica'} onChange={(e) => setExportFormat(e.target.value)} className="w-4 h-4" aria-label="iCal format" />
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">iCal (.ics)</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Import into Apple/Google Calendar</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                    <input type="radio" name="format" value="csv" checked={exportFormat === 'csv'} onChange={(e) => setExportFormat(e.target.value)} className="w-4 h-4" aria-label="CSV format" />
                    <div>
                  ...
                  <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                    <input type="radio" name="format" value="json" checked={exportFormat === 'json'} onChange={(e) => setExportFormat(e.target.value)} className="w-4 h-4" aria-label="JSON format" />
                    <div>
                ...
                <button onClick={exportCalendar} className="w-full mt-6 py-3 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors">Export {exportFormat.toUpperCase()}</button>
              </div>
            </div>
          </div>
        )}
