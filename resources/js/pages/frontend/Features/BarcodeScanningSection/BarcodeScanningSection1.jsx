...
const BarcodeScanningSection1 = ({ config }) => {
  ...
  return (
    <section className="relative py-12 sm:py-16 md:py-20 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      <div className="absolute top-40 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-cyan-200 dark:bg-cyan-900/20 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse" />
      <div className="absolute bottom-40 right-0 w-48 h-48 sm:w-72 sm:h-72 bg-blue-200 dark:bg-blue-900/20 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse delay-1000" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {config?.benefits && config.benefits.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 mb-12 sm:mb-16">
            {config.benefits.map((benefit, index) => (
              <div
                key={index}
                className="text-center p-5 sm:p-6 bg-linear-to-br from-cyan-50 to-blue-50 dark:from-gray-800 dark:to-gray-800/50 rounded-lg sm:rounded-xl border border-cyan-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
              >
                <div className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 bg-cyan-100 dark:bg-cyan-900/30 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 md:mb-4">
                  {getFeatureIcon(benefit.icon, "w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 text-cyan-600 dark:text-cyan-400")}
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
          {config?.features?.map((feature) => (
            ...
            >
              {/* Feature Icon */}
              <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-cyan-50 dark:bg-gray-700 rounded-lg sm:rounded-xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300">
                {getFeatureIcon(feature.icon, "w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-8 text-cyan-600 dark:text-cyan-400")}
              </div>
              ...
              <Link
                href={feature.link}
                className="inline-flex items-center text-cyan-600 dark:text-cyan-400 font-semibold hover:text-cyan-700 dark:hover:text-cyan-300 transition-colors duration-300 group/link text-xs sm:text-sm"
              ...
              <div className="absolute inset-0 rounded-lg sm:rounded-xl bg-linear-to-br from-cyan-600/0 via-cyan-600/0 to-cyan-600/5 dark:from-cyan-400/0 dark:via-cyan-400/0 dark:to-cyan-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
            </div>
          ))}
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {config.barcodeTypes.map((type, index) => (
                ...
                >
                  ...
                  <div className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white">{type.name}</div>
