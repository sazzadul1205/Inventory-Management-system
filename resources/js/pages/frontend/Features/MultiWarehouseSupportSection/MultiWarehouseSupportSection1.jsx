...
const MultiWarehouseSupportSection1 = ({ config }) => {
  ...
  const getFeatureIcon = (iconName, className = "w-8 h-8") => {
    switch (iconName) {
      case 'warehouse':
        return <HiOutlineOfficeBuilding className={`${className} text-blue-600 dark:text-blue-400`} aria-hidden="true" />;
      case 'globe':
        return <HiOutlineGlobeAlt className={`${className} text-blue-600 dark:text-blue-400`} aria-hidden="true" />;
      case 'sync':
      ...
      case 'location':
        return <HiOutlineLocationMarker className={`${className} text-blue-600 dark:text-blue-400`} aria-hidden="true" />;
      case 'inventory':
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-40 left-0 w-72 h-72 bg-purple-200 dark:bg-purple-900/20 rounded-full mix-blend-multiply filter blur-3xl animate-blob" aria-hidden="true"></div>
        ...
        >
          {config?.features?.map((feature) => (
            ...
            >
              ...
              <div className="absolute inset-0 rounded-2xl bg-linear-to-br from-purple-600/0 via-purple-600/0 to-purple-600/5 dark:from-purple-400/0 dark:via-purple-400/0 dark:to-purple-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" aria-hidden="true"></div>
            </div>
          ))}
        </div>
          ...
          <div className="mt-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {config?.warehouses?.map((warehouse, index) => (
                <div key={index} className="relative bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex items-center justify-center">
                      <HiOutlineLocationMarker className="w-5 h-5 text-purple-600 dark:text-purple-400" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900 dark:text-white">{warehouse.location}</h4>
