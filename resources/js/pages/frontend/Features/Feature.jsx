...
const PageHero = ({ heroData }) => {
  ...
  const highlightedText = heroData?.highlightedText || 'Features';
  ...
  return (
    <section
      className="relative bg-linear-to-r from-blue-600 to-blue-800 dark:from-blue-900 dark:to-blue-950 text-white overflow-hidden"
      aria-label="Features page hero section"
    >
      ...
      <div className="container mx-auto px-4 py-20 md:py-28 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <nav aria-label="Breadcrumb" className="flex items-center justify-center gap-2 text-sm text-blue-100 dark:text-blue-200 mb-6">
            ...
            <span className="text-white dark:text-white" aria-current="page">Features</span>
          </nav>
...
const Feature = ({ pageData = { meta: {}, sections: [] } }) => {
  ...
  const sectionsWithDisplayName = sections.map(section => {
    ...
    return {
      ...section,
      type: normalizedType,
      displayName: displayNames[normalizedType] || normalizedType
    };
  });
    ...
    <FrontEnd_Layout>
      ...
      >
        ...
        {sections?.map((section, index) => {
          ...
          const sectionId = `${normalizedType}-${_id || index}`;

          // Get the component from registry
          const SectionComponent =
            sectionRegistry[normalizedType]?.[variant] ||
            sectionRegistry[normalizedType]?.variant1;

          // Get skeleton component and props based on section type
          const SkeletonComponent = skeletonRegistry[normalizedType] || skeletonRegistry.default;
          const skeletonProps = getSkeletonProps(normalizedType, variant, config);
          ...
          if (!SectionComponent) {
            return (
              <section
                ...
                role="alert"
                aria-label={`Error: Section ${normalizedType} not found`}
              >
                <h3 className="text-red-700 dark:text-red-400 font-bold">Error: Section not found</h3>
                <p className="text-red-600 dark:text-red-300">Type: {normalizedType}, Variant: {variant || 'default'}</p>
          ...
          return (
            <section
              key={sectionId}
              id={`section-${normalizedType}`}
              aria-label={`${normalizedType.replace(/([A-Z])/g, ' $1').trim()} section`}
              data-section-type={normalizedType}
            ...
            >
              <Suspense
                fallback={
                  <SkeletonComponent
                    {...skeletonProps}
                    aria-label={`Loading ${normalizedType} section content`}
                  />
                }
              >
                <SectionComponent
