...
/**
 * Global Reach Section - Borderless Operations Infrastructure Hub
 * 
 * Unique design elements:
 * - Interactive region filter for global offices
 * - Office location cards with team size and hours
 * - Language support showcase
 * - Global data center region visualization
 * - Data residency and compliance badges
 * - Local latency metrics by region
...
// React Icons - Global infrastructure focused set
import {
  BsGlobe2,
  BsCloudCheck
} from 'react-icons/bs';
...
import { MdVerified, MdOutlineDataSaverOn, MdOutlineLocationOn } from 'react-icons/md';
import {
...
const GlobalReachSection1 = ({ config }) => {
  ...
  const regions = useMemo(() => config?.regions || [], [config?.regions]); // config?.regions || [];
  ...
  const getIcon = useCallback((iconName, className = "w-5 h-5") => {
    const icons = {
      ...
      HiOutlineFilter, HiOutlineBookmark, HiOutlinePrinter, HiOutlineDownload, HiOutlineShieldCheck,      
      TbCloudDataConnection, TbDatabase, TbUsers, TbLanguage, TbWorld, TbShieldCheck, TbHeadset,
      FaGlobeAmericas, TbCloud, TbServer, TbGlobe,
      FaGlobeEurope, FaGlobeAsia, FaLanguage,
      MdVerified, MdOutlineDataSaverOn, MdOutlineLocationOn,
      GiWorld, GiEarthAmerica, FaEarthEurope, GiEarthAsiaOceania,
      BsGlobe2, BsCloudCheck,
    };
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Global Infrastructure & Reach"
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          >
            ...
            <span className={`text-sm font-medium ${config?.badge?.textColor || 'text-indigo-700 dark:text-indigo-300'}`}>
              {config?.badge?.text || "Borderless Operations"}
            </span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6">
            ...
            <span className={`bg-linear-to-r ${config?.title?.highlightGradient || 'from-indigo-600 to-purple-600'} bg-clip-text text-transparent`}>
              {config?.title?.highlightedText || 'Infrastructure'}
            </span>{' '}
            {config?.title?.suffix || 'Powering Local Excellence'}
          </h2>
          <p className="text-xl text-gray-600 dark:text-gray-300">
            {config?.description || "Your business spans continents, and so do we. With data centers across North America, Europe, Asia Pacific, and Latin America, we deliver sub-50ms latency and local compliance wherever you operate."}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex justify-center mb-2 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform">
        ...
        <div className="mb-16 bg-linear-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-2xl p-8 text-center border border-indigo-100 dark:border-indigo-800">
          <div className="flex justify-center mb-4">
            <div className="w-20 h-20 bg-indigo-100 dark:bg-indigo-900/30 rounded-full flex items-center justify-center">
              {getIcon("GiWorld", "w-10 h-10 text-indigo-600")}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {regions.map((region, index) => (
              <div
                ...
                className="bg-white dark:bg-gray-800 rounded-xl p-4 cursor-pointer hover:shadow-lg transition-all duration-300 border border-gray-100 dark:border-gray-700"
                role="button"
                tabIndex={0}
                onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setSelectedRegion(region.id === selectedRegion ? null : region.id)}
              >
                <div className="flex justify-center mb-2 text-indigo-600 dark:text-indigo-400 text-2xl">
            ...
            >
              <div className="h-32 bg-linear-to-r from-indigo-500 to-purple-600 relative">
                ...
                <div className="absolute top-4 right-4 bg-white/20 rounded-full px-2 py-1 text-xs text-white">
                  {getRegionName(office.region)}
                </div>
              </div>
              <div className="p-6">
                <div className="flex items-start gap-3 mb-3">
                  {getIcon("MdOutlineLocationOn", "w-5 h-5 text-indigo-500 mt-0.5 shrink-0")}
                  <div className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                    {office.address}
                  </div>
                </div>
                <div className="flex items-start gap-3 mb-3">
                  {getIcon("HiOutlineClock", "w-5 h-5 text-indigo-500 mt-0.5 shrink-0")}
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                    {office.hours}
                  </div>
                </div>
                <div className="flex items-start gap-3 mb-4">
                  {getIcon("HiOutlineUsers", "w-5 h-5 text-indigo-500 mt-0.5 shrink-0")}
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                ...
                {office.mapLink && (
                  <Link
                    href={office.mapLink}
                    className="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 text-sm font-semibold hover:gap-2 transition-all duration-200 group"
                  >
                    View on Map
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-12 text-center border border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-center gap-3 mb-4">
            {getIcon("FaLanguage", "w-6 h-6 text-indigo-600")}
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Global Language Support</h3>
          </div>
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {languages.map((lang, index) => (
              <span key={index} className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-sm font-medium">
                {lang}
              </span>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <h3 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white text-center mb-4">
            Global Infrastructure FAQs
          </h3>
          <p className="text-center text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
            Everything you need to know about our global presence, data residency, and local support.
          </p>
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <div className="relative">
              ...
              <input
                type="text"
                ref={searchRef}
                placeholder="Search by topic (regions, compliance, support)..."
                value={searchQuery}
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
            <div className="flex gap-2">
              ...
              <select
                ...
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 transition-all text-sm"
                              ...
                              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                  <div className="flex items-center gap-4">
                                    <span className="text-xs text-gray-500 dark:text-gray-400">Was this global infrastructure answer helpful?</span>
                                    <button
          ...
          {filteredFaqs.length === 0 && searchQuery && (
            <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl mb-12">
              ...
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No global infrastructure questions found</h3>
        ...
        <div className="text-center">
          <div className="inline-flex flex-col sm:flex-row items-center gap-5 p-6 bg-linear-to-r from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-indigo-100 dark:border-gray-700">
            <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-full flex items-center justify-center">
              {getIcon("TbGlobe", "w-6 h-6 text-indigo-600")}
            </div>
