...
const PartnerDirectorySection2 = ({ config }) => {
    ...
    const getProgramConfig = (program) => {
        const configs = {
            ...
            'solution': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'briefcase', label: 'Solution Partner', borderColor: 'border-purple-200 dark:border-purple-800' },
            'consulting': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'users', label: 'Consulting Partner', borderColor: 'border-green-200 dark:border-green-800' },
            'reseller': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'globe', label: 'Reseller Partner', borderColor: 'border-orange-200 dark:border-orange-800' },
            'integration': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'code', label: 'Integration Partner', borderColor: 'border-indigo-200 dark:border-indigo-800' }
        };
    ...
    const getFilteredPartners = useCallback(() => {
        ...
        if (sortBy === 'featured') {
            ...
            partners = [...partners].sort((a, b) => (b.rating || 0) - (a.rating || 0));
        } else if (sortBy === 'name') {
            partners = [...partners].sort((a, b) => a.name.localeCompare(b.name));
        }

        return partners;
    }, [config?.partners, searchQuery, selectedProgram, selectedRegion, selectedIndustry, selectedTier, sortBy]);
    ...
    // Top rated partners
    const topRatedPartners = [...(config?.partners || [])]
        .sort((a, b) => (b.rating || 0) - (a.rating || 0))
        .slice(0, 4);

    // Active filters count
    const activeFiltersCount = [selectedProgram !== 'all', selectedRegion !== 'all', selectedIndustry !== 'all', selectedTier !== 'all', searchQuery !== ''].filter(Boolean).length;
                    ...
                    {programTypes.map((program) => (
                        ...
                        >
                            ...
                            {program.count !== undefined && (
                                <span className={`ml-1 px-2 py-0.5 rounded-full text-xs ${selectedProgram === program.id
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                <h3 className="font-semibold text-gray-900 dark:text-white text-sm line-clamp-1">{partner.name}</h3>
                                    ...
                                    {partner.rating && (
                                        <div className="flex items-center gap-2 mt-2 mb-3">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="text-sm font-semibold text-gray-900 dark:text-white">{partner.rating}</span>
                                            </div>
                                        ...
                                        {partner.industries && partner.industries.length > 0 && (
                                            <div className="flex gap-1">
                                                {partner.industries.slice(0, 2).map((industry, idx) => (
                                                    <span key={idx} className="text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full">
