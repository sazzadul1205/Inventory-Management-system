...
const IntegrationPartnersSection2 = ({ config }) => {
    ...
    const [sortBy, setSortBy] = useState('featured'); // featured, name, category
    ...
    const getCategoryConfig = (category) => {
        const configs = {
            'erp': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'building', label: 'ERP Systems', borderColor: 'border-blue-200 dark:border-blue-800' },
            'ecommerce': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'globe', label: 'E-commerce Platforms', borderColor: 'border-purple-200 dark:border-purple-800' },
            'crm': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'users', label: 'CRM Systems', borderColor: 'border-green-200 dark:border-green-800' },
            'wms': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'building', label: 'WMS Providers', borderColor: 'border-orange-200 dark:border-orange-800' },
            'tms': { color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'globe', label: 'TMS Providers', borderColor: 'border-red-200 dark:border-red-800' },
            'payment': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'credit', label: 'Payment Gateways', borderColor: 'border-indigo-200 dark:border-indigo-800' }
        };
    ...
    const getTypeConfig = (type) => {
        const configs = {
            'api': { color: 'bg-blue-100 text-blue-700', label: 'REST API', badge: '🔌 API', icon: 'code' },
            'pre-built': { color: 'bg-green-100 text-green-700', label: 'Pre-built Connector', badge: '⚡ Pre-built', icon: 'link' },
            'sdk': { color: 'bg-purple-100 text-purple-700', label: 'SDK', badge: '📦 SDK', icon: 'template' },
            'webhook': { color: 'bg-orange-100 text-orange-700', label: 'Webhook', badge: '🪝 Webhook', icon: 'refresh' }
        };
    ...
    const getCertificationConfig = (level) => {
        const configs = {
            'certified': { color: 'bg-green-100 text-green-700', badge: '✓ Certified', icon: 'badge' },
            'advanced': { color: 'bg-blue-100 text-blue-700', badge: '⭐ Advanced', icon: 'star' },
            'premier': { color: 'bg-yellow-100 text-yellow-700', badge: '🏆 Premier', icon: 'trophy' }
        };
    ...
    const getFilteredPartners = useCallback(() => {
        ...
        if (sortBy === 'featured') {
            ...
            partners = [...partners].sort((a, b) => (a.category || '').localeCompare(b.category || ''));
        }

        return partners;
    }, [config?.partners, searchQuery, selectedCategory, selectedType, selectedCertification, sortBy]);

    const filteredPartners = getFilteredPartners();
    const categories = config?.categories || [
        { id: 'all', label: 'All Integrations', icon: 'link', count: config?.partners?.length || 0 },
    ...
    const activeFiltersCount = [selectedCategory !== 'all', selectedType !== 'all', selectedCertification !== 'all', searchQuery !== ''].filter(Boolean).length;
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
                    <div>
                        <div className="inline-flex items-center gap-2 bg-purple-50 dark:bg-purple-900/30 rounded-full px-4 py-2 mb-4">
                            <HiOutlineLink className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-8">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            placeholder={config?.searchPlaceholder || "Search integrations by name, category, or technology..."}
                ...
                {showFilters && (
                    <div className="mb-8 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                        <div className="grid md:grid-cols-4 gap-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Certification Level</label>
                                <select
                            ...
                            >
                                ...
                                <div className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                                        <Link href={partner.link} className="hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
                                            {partner.name}
                                        </Link>
                                    </h3>
                ...
                {config?.showNewsletter && (
                    <div className="mt-8 bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-200 dark:border-gray-700 text-center">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-2xl mx-auto">
                            {config?.newsletter?.description || "Subscribe to receive updates on new integrations, API changes, and developer resources."}
                        </p>
