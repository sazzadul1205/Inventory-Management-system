...
const TechnologyPartnersSection3 = ({ config }) => {
    ...
    const getCategoryConfig = (category) => {
        const configs = {
            'cloud': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'cloud', label: 'Cloud Infrastructure', gradient: 'from-blue-500 to-blue-600' },
            'ai-ml': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'chip', label: 'AI & ML', gradient: 'from-purple-500 to-purple-600' },
            'analytics': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'chart', label: 'Analytics & BI', gradient: 'from-green-500 to-green-600' },
            'integration': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'code', label: 'Integration', gradient: 'from-orange-500 to-orange-600' },
            ...
            'iot': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'wifi', label: 'IoT & Sensors', gradient: 'from-indigo-500 to-indigo-600' }
        };
    ...
    const getIntegrationConfig = (integration) => {
        const configs = {
            'api': { color: 'bg-blue-100 text-blue-700', label: 'REST API', icon: 'code', badge: '🔌 REST API', gradient: 'from-blue-500 to-blue-600' },
            'pre-built': { color: 'bg-green-100 text-green-700', label: 'Pre-built Connector', icon: 'link', badge: '⚡ Pre-built', gradient: 'from-green-500 to-green-600' },
            'sdk': { color: 'bg-purple-100 text-purple-700', label: 'SDK', icon: 'template', badge: '📦 SDK', gradient: 'from-purple-500 to-purple-600' },
            'webhook': { color: 'bg-orange-100 text-orange-700', label: 'Webhook', icon: 'refresh', badge: '🪝 Webhook', gradient: 'from-orange-500 to-orange-600' }
        };
    ...
    const getFilteredPartners = useCallback(() => {
        ...
        return partners;
    }, [config?.partners, searchQuery, selectedCategory, selectedIntegration, activeTab, favoritePartners]);
    ...
    // Integration guides
    const integrationGuides = config?.integrationGuides || [];

    // Active filters count
    const activeFiltersCount = [selectedCategory !== 'all', selectedIntegration !== 'all', searchQuery !== ''].filter(Boolean).length;
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-4xl mx-auto mb-12">
                    ...
                    <p className="text-xl text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
                        {config?.description || "Discover and integrate with leading technology partners to build a complete supply chain ecosystem. Access pre-built connectors, APIs, and SDKs."}
                    </p>
                ...
                {activeTab === 'all' && featuredPartners.length > 0 && (
                    <div className="relative mb-16">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Featured Technology Partners</h2>
                                            ...
                                            <div className="relative h-96 rounded-3xl overflow-hidden">
                                                ...
                                                <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                                                    <div className="flex flex-wrap gap-2 mb-3">
                                                        ...
                                                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getIntegrationConfig(partner.integrationType).color}`}>
                                                            {getIntegrationConfig(partner.integrationType).badge}
                                                        </span>
                                                    </div>
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {partner.demoUrl && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setCurrentVideo(partner.demoUrl); setShowVideoModal(true); }}
                                                    className="text-gray-400 hover:text-blue-600 transition-colors"
                                                >
                                        ...
                                        <span className="text-blue-600 text-xs font-semibold hover:underline">Learn More →</span>
                                    </div>
                                </div>
                            </div>
                        );
                            ...
                            <div className="p-6">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-900 dark:text-white">{getIntegrationConfig(selectedPartner.integrationType).label}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {selectedPartner.integrationCapabilities.map((cap, idx) => (
                                                <span key={idx} className="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full">{cap}</span>
                                            ))}
                                        </div>
                                    </div>
                                )}
