...
import {
    ...
    HiOutlineZoomIn,
    HiOutlineVolumeUp,
    HiOutlineQrcode as HiOutlineQrcodeIcon,
    HiOutlinePrinter as HiOutlinePrinterIcon,
    ...
    HiOutlineDeviceMobile as HiOutlineDeviceMobileIcon,
    HiOutlineWifi as HiOutlineWifiIcon
} from 'react-icons/hi';
import {
    HiArrowRight,
    HiOutlineTrophy,
...
const CompanyCultureSection3 = ({ config }) => {
    ...
    const getIcon = (iconName, className = "w-5 h-5") => {
        const icons = {
            ...
            printerIcon: <HiOutlinePrinterIcon className={className} />,
            shareAlt: <HiOutlineShareAlt className={className} />,
            duplicate: <HiOutlineDuplicate className={className} />,
            qrcode: <HiOutlineQrcode className={className} />,
            qrcodeIcon: <HiOutlineQrcodeIcon className={className} />,
            ...
            mobileIcon: <HiOutlineDeviceMobileIcon className={className} />,
            wifi: <HiOutlineWifi className={className} />,
            wifiIcon: <HiOutlineWifiIcon className={className} />,
    ...
    const getCategoryConfig = (category) => {
        const configs = {
            ...
            'benefits': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'gift', label: 'Benefits & Perks', gradient: 'from-green-500 to-green-600' },
    ...
    const handleFavoriteStory = (storyId) => {
    ...
    // Carousel navigation for values
    const nextSlide = useCallback(() => {
        setCurrentSlide((prev) => (prev + 1) % (config?.coreValues?.length || 1));
    }, [config?.coreValues?.length]);

    const prevSlide = useCallback(() => {
        setCurrentSlide((prev) => (prev - 1 + (config?.coreValues?.length || 1)) % (config?.coreValues?.length || 1));
    }, [config?.coreValues?.length]);

    // Auto-play carousel
    useEffect(() => {
        if (config?.autoPlayCarousel && config?.coreValues?.length > 1 && activeTab === 'values') {
            const interval = setInterval(() => {
                nextSlide();
            }, 6000);
            return () => clearInterval(interval);
        }
    }, [config?.autoPlayCarousel, config?.coreValues?.length, activeTab, nextSlide]);
    ...
    const getFilteredStories = useCallback(() => {
        ...
        return stories;
    }, [config?.stories, searchQuery, selectedCategory, selectedValue, activeTab, favoriteStories]);
    ...
    const coreValues = config?.coreValues || [
        { title: "Customer First", description: "We put our customers at the center of everything we do. Their success is our success.", icon: "heart", color: "from-red-500 to-pink-500", videoUrl: "/videos/customer-first.mp4" },
        { title: "Innovation", description: "We embrace curiosity and push boundaries to create breakthrough solutions that transform supply chains.", icon: "lightbulb", color: "from-yellow-500 to-orange-500", videoUrl: "/videos/innovation.mp4" },
        { title: "Integrity", description: "We act with honesty, transparency, and accountability in everything we do.", icon: "badge", color: "from-green-500 to-emerald-500", videoUrl: "/videos/integrity.mp4" },
        { title: "Collaboration", description: "We achieve more together through teamwork and mutual respect across all boundaries.", icon: "handshake", color: "from-blue-500 to-cyan-500", videoUrl: "/videos/collaboration.mp4" },
        { title: "Excellence", description: "We strive for excellence in everything we do, delivering quality and value to our customers.", icon: "trophy", color: "from-purple-500 to-indigo-500", videoUrl: "/videos/excellence.mp4" },
        { title: "Inclusion", description: "We celebrate diversity and create a culture where everyone belongs and can thrive.", icon: "users", color: "from-pink-500 to-rose-500", videoUrl: "/videos/inclusion.mp4" }
    ];
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        {stats.map((stat, idx) => (
                            <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-5 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
                                <div className="w-8 h-8 rounded-full bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center">
                                    {getIcon(stat.icon, "w-4 h-4 text-pink-600")}
                                </div>
                    ...
                    <div className="mb-12">
                        <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-xl border border-gray-200 dark:border-gray-700">
                            <div className="text-center mb-8">
                                <HiOutlineAcademicCap className="w-12 h-12 mx-auto text-pink-600 mb-4" />
                                ...
                                <div className="text-center py-8">
                                    <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                                        {quizScore >= 2 ? (
                                            ...
                                            <HiOutlineAcademicCap className="w-10 h-10 text-orange-600" />
                                        )}
                                    </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 mb-8">
                            <div className="flex-1 relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500"
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    ...
                                    {values.filter(v => v.id !== 'all').map(val => (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="relative h-48 overflow-hidden">
                                            ...
                                            >
                                                <HiOutlineBookmark className={`w-4 h-4 ${isFavorite ? 'fill-yellow-500 text-yellow-500' : 'text-gray-600'}`} />
                                            </button>
                                        </div>
                                        ...
                                        <div className="p-5">
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                                                <div className="flex items-center gap-2 text-xs text-gray-500">
                                                    <HiOutlineCalendarIcon className="w-3 h-3" />
                ...
                {(activeTab === 'all' || activeTab === 'featured' || activeTab === 'favorites') && filteredStories.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        {activeTab === 'favorites' && (
                            <button onClick={() => setActiveTab('all')} className="mt-4 text-pink-600 hover:underline">
                                Browse All Stories
                            </button>
                        )}
                        {activeFiltersCount > 0 && (
                            <button onClick={clearAllFilters} className="mt-4 text-pink-600 hover:underline ml-4">
                                Clear all filters
                            </button>
                        )}
                    </div>
                ...
                {showStoryModal && selectedStory && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowStoryModal(false)}>
                        <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl max-h-90vh overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                            <div className={`sticky top-0 p-6 bg-linear-to-r ${getCategoryConfig(selectedStory.category).gradient} text-white`}>
                                <div className="flex items-center justify-between">
                                    ...
                                    <button onClick={() => setShowStoryModal(false)} className="text-white/80 hover:text-white">
                            ...
                            <div className="p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <img src={selectedStory.author?.avatar} alt={selectedStory.author?.name} className="w-12 h-12 rounded-full object-cover" />
                                    <div>
                                        <p className="font-semibold text-gray-900 dark:text-white">{selectedStory.author?.name}</p>
                                        <p className="text-sm text-gray-500">{selectedStory.author?.role}</p>
                                    </div>
                                </div>
                                <p className="text-gray-600 dark:text-gray-400 mb-4">{selectedStory.content}</p>
                                {selectedStory.quote && (
                                    <div className="mb-4 p-4 bg-pink-50 dark:bg-pink-900/20 rounded-xl border-l-4 border-pink-500">
                                        ...
                                        <p className="text-sm italic text-gray-700 dark:text-gray-300">"{selectedStory.quote.text}"</p>
                                        <p className="text-xs text-gray-500 mt-2">— {selectedStory.quote.author}</p>
                                    </div>
                                )}
                                <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <button
                                        onClick={() => handleFavoriteStory(selectedStory.id)}
                                        className={`flex-1 py-2 rounded-lg font-semibold transition-colors ${favoriteStories.includes(selectedStory.id) ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-600'}`}
                                    >
                                        <HiOutlineBookmark className="w-5 h-5 inline mr-2" />
                                    ...
                                    <button className="flex-1 bg-pink-600 text-white py-2 rounded-lg font-semibold hover:bg-pink-700 transition-colors">
                                        <HiOutlineShareAlt className="w-5 h-5 inline mr-2" />
                                        Share
                                    </button>
                                </div>
                            </div>
