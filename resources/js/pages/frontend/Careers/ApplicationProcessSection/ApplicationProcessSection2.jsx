...
/**
 * Application Process Section II - Detailed Application Guide with Interactive Timeline
 *
 * Unique Design Elements:
 * - Stats Cards with Trend Indicators (Weeks to Offer, Satisfaction, Response Time, Offer Acceptance)
 * - Interactive Timeline with Step Navigation (Desktop) / Accordion (Mobile)
 * - Step Details with Pro Tips and Checklist
 * - Application Resources Grid with Link Cards
 * - Comprehensive Application Form with Validation
 * - FAQ Accordion Section with Expandable Answers
 * - Application Tips Modal with Success Tips
 * - Position Dropdown with Multiple Options
...
const ApplicationProcessSection2 = ({ config }) => {
  ...
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', phone: '', position: '', experience: '', location: '', resume: null, portfolio: '', referralSource: '', message: '', agreeToTerms: false, subscribeToUpdates: false });
  ...
  /**
   * Returns step configuration with color, icon, and status
   */
  const getStepConfig = useCallback((stepId) => {
    const configs = {
      ...
      3: { color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', borderColor: 'border-emerald-200 dark:border-emerald-800', icon: 'code', status: 'pending' },
   ...
   * Set active step
   */
  const setActiveStepHandler = useCallback((stepId) => {
              ...
              {steps.map((step) => {
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center text-center w-1/5">
                    <button
                      onClick={() => setActiveStepHandler(step.id)}
                    ...
                    <h3
                      className={`mt-3 font-semibold ${activeStep === step.id ? 'text-blue-600' : 'text-gray-700 dark:text-gray-300'
                        }`}
                    >
                      {step.title}
                    </h3>
          ...
          <div className="md:hidden space-y-4">
            {steps.map((step) => {
              ...
              return (
                ...
                >
                  <button
                    onClick={() => setActiveStepHandler(isActive ? null : step.id)}
                  ...
                  >
                    ...
                    {getIcon('chevron-down', `w-5 h-5 text-gray-400 transition-transform ${isActive ? 'rotate-180' : ''}`)}
                  </button>
                  {isActive && (
                    <div className="p-4 pt-0 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
          ...
          {formSubmitted ? (
            <div className="text-center py-12 animate-fadeIn">
              ...
              <button
                ...
                className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 justify-center"
                aria-label="View application tips"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.position
                      ? 'border-red-500'
                      : 'border-gray-200 dark:border-gray-600'
              ...
              <div className="p-6">
                <ul className="space-y-3">
                  ...
                  <li className="flex items-start gap-3">
                    {getIcon('check', 'w-5 h-5 text-emerald-500 mt-0.5 shrink-0')}
                    <span className="text-gray-700 dark:text-gray-300">
                      Include a link to your portfolio or GitHub if applicable
                    </span>
                  </li>
                  <li className="flex items-start gap-3">
                    {getIcon('check', 'w-5 h-5 text-emerald-500 mt-0.5 shrink-0')}
