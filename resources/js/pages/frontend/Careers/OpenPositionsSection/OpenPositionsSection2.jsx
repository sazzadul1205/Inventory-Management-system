...
const OpenPositionsSection2 = ({ config }) => {
    ...
    const [sortBy, setSortBy] = useState('date'); // date, title, salary
    ...
    const getDepartmentConfig = (department) => {
        const configs = {
            ...
            'sales': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'chart', label: 'Sales', borderColor: 'border-green-200 dark:border-green-800' },
    ...
    const getFilteredPositions = useCallback(() => {
        ...
        if (sortBy === 'date') {
            positions = [...positions].sort((a, b) => new Date(b.postedDate) - new Date(a.postedDate));
        } else if (sortBy === 'title') {
            positions = [...positions].sort((a, b) => a.title.localeCompare(b.title));
        } else if (sortBy === 'salary') {
            positions = [...positions].sort((a, b) => {
                const aSalary = parseInt(a.salary) || 0;
                const bSalary = parseInt(b.salary) || 0;
                return bSalary - aSalary;
            });
        }

        return positions;
    }, [config?.positions, searchQuery, selectedDepartment, selectedLocation, selectedType, selectedExperience, sortBy]);
    ...
    // Top positions by popularity
    const topPositions = [...(config?.positions || [])]
        .sort((a, b) => (b.views || 0) - (a.views || 0))
        .slice(0, 4);

    // Active filters count
    const activeFiltersCount = [selectedDepartment !== 'all', selectedLocation !== 'all', selectedType !== 'all', selectedExperience !== 'all', searchQuery !== ''].filter(Boolean).length;
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
                    <div>
                        <div className="inline-flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 rounded-full px-4 py-2 mb-4">
                            <HiOutlineBriefcase className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            {topPositions.map((position) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-10 h-10 rounded-lg ${deptConfig.color} flex items-center justify-center`}>
                                            {getIcon(deptConfig.icon, "w-5 h-5")}
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                <h3 className="font-semibold text-gray-900 dark:text-white text-sm line-clamp-1">{position.title}</h3>
                                            </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`relative overflow-hidden ${viewMode === 'list' ? 'md:w-64 md:shrink-0' : ''}`}>
                                    <div className={`h-24 ${viewMode === 'list' ? 'h-full' : ''} bg-linear-to-r from-blue-600 to-purple-600 p-4 text-white`}>
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <h3 className="font-bold text-lg line-clamp-2">{position.title}</h3>
                                            ...
                                            <button
                                                ...
                                                className={`transition-colors ${isSaved ? 'text-yellow-500' : 'text-white/70 hover:text-yellow-500'}`}
                                            >
