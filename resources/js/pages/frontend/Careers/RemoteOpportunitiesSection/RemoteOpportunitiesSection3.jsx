...
/**
 * Remote Opportunities Section III - Full Remote Work Hub with Stories & Application
 *
 * Unique Design Elements:
 * - Multi-tab UI (All Jobs, Featured, Favorites, Remote Stories)
 * - Remote Stories Carousel with Auto-play and Manual Navigation
 ...
 * - Level Badges with Emoji-style Icons
 ...
 * - Video Modal for Remote Stories
 *
...
const RemoteOpportunitiesSection3 = ({ config }) => {
  ...
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', phone: '', location: '', yearsExperience: '', currentRole: '', linkedin: '', portfolio: '', resume: null, coverLetter: null, positionId: '', hearAbout: '', availability: '', message: '', agreeToTerms: false });
  ...
  const remoteStories = useMemo(() => config?.remoteStories || [], [config?.remoteStories]);
  ...
  const getDepartmentConfig = useCallback((departmentId) => {
    const configs = {
      ...
      design: { color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', icon: 'template', label: 'Design', gradient: 'from-emerald-500 to-emerald-600' },
  ...
  const openApplyModal = useCallback((job) => {
    ...
    setFormData((prev) => ({ ...prev, positionId: job.id }));
    setFormStep(1);
    setErrors({});
  ...
  const generateApplicationId = useCallback(() => {
    return `REM-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
  }, []);
  ...
  // ==================== CAROUSEL NAVIGATION ====================
  const storiesCount = remoteStories.length;
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs;
  }, [allJobs, searchQuery, selectedDepartment, selectedLevel, selectedRegion, selectedType, activeTab, favoriteJobs]);
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {job.skills.length > 3 && (
                              <button
                                ...
                                aria-label={`Show ${job.skills.length - 3} more skills`}
                              >
                                +{job.skills.length - 3}
                              </button>
                            )}
        ...
        {/* ==================== EMPTY STATE ==================== */}
        {(activeTab === 'jobs' || activeTab === 'featured' || activeTab === 'favorites') && filteredJobs.length === 0 && (
          <div className="text-center py-16 bg-gray-50 dark:bg-gray-800/50 rounded-2xl mb-12">
            ...
            {activeTab === 'favorites' && (
              <button onClick={() => setActiveTab('jobs')} className="mt-4 text-blue-600 dark:text-blue-400 font-semibold hover:underline">
                Browse All Jobs
              </button>
            )}
            {activeFiltersCount > 0 && (
                ...
                {formSubmitted ? (
                  <div className="text-center py-12 animate-fadeIn">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      Thank you for applying to {selectedJobForApply.title}. Our recruiting team will review your application.
                    </p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={handleInputChange}
                              className={`w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border rounded-lg ${errors.location
                                ? 'border-red-500'
                                : 'border-gray-200 dark:border-gray-600'
                                } text-gray-900 dark:text-white`}
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={handleInputChange}
                            className={`w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border rounded-lg ${errors.yearsExperience
                              ? 'border-red-500'
                              : 'border-gray-200 dark:border-gray-600'
                              } text-gray-900 dark:text-white`}
                          >
