...
const RemoteOpportunitiesSection1 = ({ config }) => {
    ...
    const getDepartmentConfig = (department) => {
        const configs = {
            ...
            'product': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'briefcase', label: 'Product' },
            'design': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'template', label: 'Design' },
            'marketing': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'chart', label: 'Marketing' },
            'sales': { color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'briefcase', label: 'Sales' },
            'customer-success': { color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: 'users', label: 'Customer Success' }
        };
    ...
    const getLevelConfig = (level) => {
        const configs = {
            'entry': { color: 'bg-green-100 text-green-700', label: 'Entry Level' },
            'mid': { color: 'bg-blue-100 text-blue-700', label: 'Mid Level' },
            'senior': { color: 'bg-purple-100 text-purple-700', label: 'Senior' },
            'lead': { color: 'bg-orange-100 text-orange-700', label: 'Lead' },
            'manager': { color: 'bg-red-100 text-red-700', label: 'Manager' }
        };
        return configs[level] || { color: 'bg-gray-100 text-gray-700', label: level };
    };
    ...
    // Featured job
    const featuredJob = config?.featuredJob || filteredJobs[0];
    ...
    return (
        <section
            ...
            role="region"
            aria-label="Remote Opportunities Section"
        >
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-12">
                    ...
                    <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6">
                        {config?.title?.prefix || "Work from"} <span className="bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">{config?.title?.highlight || "Anywhere"}</span> {config?.title?.suffix || ""}
                    </h2>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getDepartmentConfig(featuredJob.department).color}`}>
                                                    {getDepartmentConfig(featuredJob.department).label}
                                                </span>
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getLevelConfig(featuredJob.level).color}`}>
                                        ...
                                        <div className="flex flex-wrap items-center gap-4 mb-6">
                                            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                                ...
                                                <span>{getRegionConfig(featuredJob.region).flag} {getRegionConfig(featuredJob.region).label}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            <button
                                                onClick={() => {
                                                    setFormData({ ...formData, position: featuredJob.title });
                                ...
                                <div className="p-6">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-xl ${deptConfig.color} flex items-center justify-center`}>
                                    ...
                                    <div className="space-y-2 mb-4">
                                        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                            <HiOutlineGlobe className="w-4 h-4" />
                                            <span>{regionConfig.flag} {regionConfig.label} ({regionConfig.timezone})</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                            <HiOutlineCalendar className="w-4 h-4" />
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                                        ...
                                        <span className="text-xs text-green-600">{job.remoteType}</span>
                                    </div>
                                </div>
                            </div>
                        );
                ...
                <div id="application-form" className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-xl border border-gray-200 dark:border-gray-700">
                    <div className="text-center mb-8">
                        <HiOutlineWifi className="w-12 h-12 mx-auto text-blue-600 dark:text-blue-400 mb-4" />
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                            {config?.formTitle || "Apply for Remote Position"}
                        </h3>
                        <p className="text-gray-600 dark:text-gray-400">
                            {config?.formDescription || "Fill out the form below and our recruiting team will review your application."}
                        </p>
                    </div>
                    ...
                    {formSubmitted ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-600 dark:text-gray-400">Thank you for your interest. Our recruiting team will review your application and reach out within 5 business days.</p>
                        </div>
                    ) : (
