// page/frontend/Home/TransportationManagementSection/TransportationManagementSection3.jsx
...
const TransportationManagementSection3 = ({ config }) => {
  ...
  return (
    <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      <div className="absolute inset-0" aria-hidden="true">
        <img
          src={config?.backgroundImage || "https://images.unsplash.com/photo-1566576912321-d58ddd7a6088?w=1920&h=800&fit=crop"}
          alt=""
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-4xl mx-auto mb-16 sm:mb-20">
          {config?.badge && (
            <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-linear-to-r from-blue-600 to-cyan-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-6 sm:mb-8 shadow-lg">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
            {config?.hero?.primaryCta?.text && (
              <Link
                ...
                className="group bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-5 sm:px-6 md:px-8 py-2.5 sm:py-3 md:py-4 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl inline-flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base"
              >
                {config.hero.primaryCta.text}
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-1.5 sm:-space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-linear-to-br from-blue-400 to-cyan-600 border-2 border-white dark:border-gray-800"
                />
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-16 sm:mb-20">
            <div className="space-y-4 sm:space-y-5 md:space-y-6">
              ...
              <Link
                ...
                className="inline-flex items-center text-blue-600 dark:text-blue-400 font-semibold hover:gap-2 transition-all duration-300 text-sm sm:text-base"
              >
                Explore AI Capabilities
                <HiArrowRight className="ml-1.5 sm:ml-2 w-3 h-3 sm:w-4 sm:h-4" />
              </Link>
            </div>

            <div className="relative mt-8 lg:mt-0">
              <div className="bg-linear-to-br from-blue-600 to-cyan-600 rounded-xl sm:rounded-2xl md:rounded-3xl p-5 sm:p-6 md:p-8 text-white">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                  <HiOutlineChip className="w-5 h-5 sm:w-6 sm:h-6" />
                ...
                <div className="mt-4 sm:mt-5 md:mt-6 pt-4 sm:pt-5 md:pt-6 border-t border-white/20">
                  <p className="text-[10px] sm:text-xs text-blue-100">Last updated: {config.intelligence.lastUpdated}</p>
                </div>
              </div>

              {/* Floating Element */}
              <div className="absolute -top-3 -right-3 sm:-top-4 sm:-right-4 w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-blue-400/30 rounded-full blur-2xl" />
            </div>
          </div>
        )}
          ...
          <div className="bg-linear-to-r from-gray-900 to-gray-800 dark:from-gray-950 dark:to-gray-900 rounded-xl sm:rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 text-white mb-16 sm:mb-20">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <div>
                ...
                <Link
                  href={config.network.link || "/network-map"}
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4">
                {config.network.regions?.map((region, idx) => (
                  <div key={idx} className="text-center p-2 sm:p-3 md:p-4 bg-white/5 rounded-lg sm:rounded-xl">
                    <div className="text-base sm:text-lg md:text-xl font-bold mb-0.5 sm:mb-1">
                      {region.percentage}
                    </div>
                    <div className="text-[8px] sm:text-[10px] text-gray-400">{region.name}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {config?.footerCta?.show && (
          <div className="bg-linear-to-r from-blue-600 to-cyan-600 dark:from-blue-500 dark:to-cyan-500 rounded-xl sm:rounded-2xl md:rounded-3xl p-8 sm:p-10 md:p-12 text-center text-white">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 md:mb-4">
...
export default TransportationManagementSection3;