    ...
    <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-4xl mx-auto mb-16 sm:mb-20">
          {config?.badge && (
            <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-linear-to-r from-indigo-600 to-purple-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-6 sm:mb-8 shadow-lg">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
            {config?.hero?.primaryCta?.text && (
              <Link
                ...
                className="group bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white px-5 sm:px-6 md:px-8 py-2.5 sm:py-3 md:py-4 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl inline-flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base"
              >
                {config.hero.primaryCta.text}
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-1.5 sm:-space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-linear-to-br from-indigo-400 to-purple-600 border-2 border-white dark:border-gray-800"
                />
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mb-16 sm:mb-20">
            {config.stats.map((stat) => (
              <div key={stat.id} className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-5 sm:p-6 shadow-md border border-gray-200 dark:border-gray-700 text-center">
                <div className="text-xl sm:text-2xl md:text-3xl font-bold text-indigo-600 dark:text-indigo-400 mb-1 sm:mb-2">
                  {stat.value}
                </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-16 sm:mb-20">
            <div className="space-y-4 sm:space-y-5 md:space-y-6">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">
                {config.innovation.title}
              </h3>
              <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
                {config.innovation.description}
              </p>
              ...
              <Link
                href={config.innovation.link || "/innovation-lab"}
            ...
            <div className="relative mt-8 lg:mt-0">
              <div className="bg-linear-to-br from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500 rounded-xl sm:rounded-2xl md:rounded-3xl p-5 sm:p-6 md:p-8 text-white">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                  <HiOutlineBeaker className="w-5 h-5 sm:w-6 sm:h-6" />
                  <span className="font-semibold text-sm sm:text-base">INNOVATION LAB</span>
                </div>
                <div className="space-y-3 sm:space-y-4">
                  {config.innovation.projects?.map((project, idx) => (
                    <div key={idx} className="flex flex-wrap items-center justify-between p-2.5 sm:p-3 bg-white/10 rounded-lg sm:rounded-xl">
                      <div>
                        <span className="text-[10px] sm:text-xs font-medium">{project.name}</span>
                        <p className="text-[8px] sm:text-[10px] text-indigo-200">{project.status}</p>
                      </div>
                ...
                <div className="mt-4 sm:mt-5 md:mt-6 pt-4 sm:pt-5 md:pt-6 border-t border-white/20">
                  <p className="text-[10px] sm:text-xs text-indigo-100">Next release: {config.innovation.nextRelease}</p>
                </div>
              </div>
        ...
        {/* Solution Showcase */}
        {config?.showcase && config.showcase.length > 0 && (
        ...
        {config?.success?.show && (
          <div className="bg-linear-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500 rounded-xl sm:rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 text-white mb-16 sm:mb-20">
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-5 md:gap-6 mb-6 sm:mb-8">
                  {config.success.stats?.map((stat, idx) => (
                    <div key={idx}>
                      ...
                      <div className="text-[10px] sm:text-xs text-indigo-100">{stat.label}</div>
                    </div>
                  ))}
                </div>

                <Link
                  href={config.success.link || "/success-stories"}
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {config.success.testimonials?.map((testimonial, idx) => (
                  <div key={idx} className="bg-white/10 rounded-lg sm:rounded-xl p-3 sm:p-4">
                    <p className="text-xs sm:text-sm font-medium mb-0.5 sm:mb-1">{testimonial.name}</p>
                    <p className="text-[8px] sm:text-[10px] text-indigo-200 mb-1 sm:mb-2">{testimonial.role}</p>
                    <p className="text-[8px] sm:text-[10px] italic">"{testimonial.quote}"</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div key={idx} className="text-center">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {category.technologies?.map((tech, techIdx) => (
                      <div key={techIdx} className="flex flex-wrap items-center justify-between gap-1 text-[10px] sm:text-xs">
                        <span className="text-gray-600 dark:text-gray-400">{tech.name}</span>
                        <span className={`text-[8px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full ${tech.badgeColor}`}>
                          {tech.status}
                        </span>
                      </div>
                    ))}
        ...
        {config?.footerCta?.show && (
          <div className="bg-linear-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500 rounded-xl sm:rounded-2xl md:rounded-3xl p-8 sm:p-10 md:p-12 text-center text-white">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 sm:gap-2 bg-white text-indigo-600 px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-semibold hover:bg-indigo-50 transition-all duration-300 transform hover:scale-105 shadow-xl text-sm sm:text-base"
            >
