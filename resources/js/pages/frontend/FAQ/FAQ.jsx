...
const PageHero = ({ heroData }) => {
  ...
  const title = heroData?.title || 'Frequently Asked';
  const highlightedText = heroData?.highlightedText || 'Questions';
  const description = heroData?.description || 'Find answers to common questions about our platform, pricing, implementation, and support. Can\'t find what you\'re looking for? Our support team is here to help.';
  ...
  return (
    <section
      className="relative bg-linear-to-r from-indigo-600 to-purple-700 dark:from-indigo-900 dark:to-purple-900 text-white overflow-hidden"
      aria-label="FAQ page hero section"
    >
      ...
      <div className="container mx-auto px-4 py-20 md:py-28 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" className="flex items-center justify-center gap-2 text-sm text-indigo-100 dark:text-indigo-200 mb-6">
            <a href="/" className="hover:text-white dark:hover:text-indigo-100 transition-colors">Home</a>
            <span aria-hidden="true">•</span>
            <span className="text-white dark:text-white" aria-current="page">FAQ</span>
          </nav>
          ...
          {/* Description */}
          <p className="text-lg md:text-xl text-indigo-100 dark:text-indigo-200 mb-8 max-w-2xl mx-auto">
            {description}
          </p>
          ...
          >
            {stats.map((stat, index) => (
              ...
              >
                ...
                <div className="text-sm text-indigo-100 dark:text-indigo-200">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
            <button
              className="px-8 py-3 bg-yellow-400 dark:bg-yellow-500 text-indigo-900 dark:text-indigo-950 font-semibold rounded-lg hover:bg-yellow-300 dark:hover:bg-yellow-400 transition-colors transform hover:scale-105 duration-200"
