...
const RegionalOfficesSection3 = ({ config }) => {
    ...
    const getRegionConfig = (region) => {
        const configs = {
            'north-america': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: 'globe', label: 'North America', flag: '🇺🇸', gradient: 'from-blue-500 to-blue-600', coordinates: { lat: 40, lng: -100 }, zoom: 4 },
            'europe': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: 'globe', label: 'Europe', flag: '🇪🇺', gradient: 'from-purple-500 to-purple-600', coordinates: { lat: 50, lng: 10 }, zoom: 4 },
            'asia-pacific': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'globe', label: 'Asia Pacific', flag: '🌏', gradient: 'from-green-500 to-green-600', coordinates: { lat: 20, lng: 100 }, zoom: 3 },
            'latin-america': { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', icon: 'globe', label: 'Latin America', flag: '🌎', gradient: 'from-orange-500 to-orange-600', coordinates: { lat: -15, lng: -60 }, zoom: 4 },
            'middle-east': { color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'globe', label: 'Middle East', flag: '🕌', gradient: 'from-red-500 to-red-600', coordinates: { lat: 25, lng: 45 }, zoom: 5 },
            'africa': { color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', icon: 'globe', label: 'Africa', flag: '🌍', gradient: 'from-emerald-500 to-emerald-600', coordinates: { lat: 0, lng: 20 }, zoom: 3 }
        };
    ...
    const getFilteredOffices = useCallback(() => {
        ...
        return offices;
    }, [config?.offices, searchQuery, selectedRegion, selectedCountry]);

    const filteredOffices = getFilteredOffices();
    const regions = config?.regions || [
        { id: 'north-america', label: 'North America', icon: 'globe', stats: { offices: 8, employees: 650, countries: 3 }, image: '/regions/north-america.jpg' },
        { id: 'europe', label: 'Europe', icon: 'globe', stats: { offices: 12, employees: 850, countries: 8 }, image: '/regions/europe.jpg' },
        { id: 'asia-pacific', label: 'Asia Pacific', icon: 'globe', stats: { offices: 10, employees: 620, countries: 6 }, image: '/regions/asia-pacific.jpg' },
        { id: 'latin-america', label: 'Latin America', icon: 'globe', stats: { offices: 5, employees: 280, countries: 4 }, image: '/regions/latin-america.jpg' },
        { id: 'middle-east', label: 'Middle East', icon: 'globe', stats: { offices: 4, employees: 220, countries: 3 }, image: '/regions/middle-east.jpg' },
        { id: 'africa', label: 'Africa', icon: 'globe', stats: { offices: 3, employees: 150, countries: 2 }, image: '/regions/africa.jpg' }
    ];
        ...
        >
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {tabs.map((tab) => (
                        ...
                        >
                            ...
                            {tab.id === 'saved' && savedOffices.length > 0 && (
                                <span className="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-xs">{savedOffices.length}</span>
                            )}
                        </button>
                    ))}
                </div>
                    ...
                    <div className="mb-12">
                        <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-xl border border-gray-200 dark:border-gray-700">
                            <div className="relative aspect-video bg-linear-to-br from-blue-50 to-purple-50 dark:from-gray-700 dark:to-gray-800 rounded-2xl overflow-hidden">
                                ...
                                >
                                    <rect width="1200" height="600" fill="url(#map-gradient-regions)" className="opacity-30" />
                                    <defs>
                                    ...
                                    {config?.offices?.filter(o => o.isRegionalHub).map((office, idx) => (
                                        ...
                                        >
                                            ...
                                            <circle
                                                cx={office.mapX}
                                                cy={office.mapY}
                                                r={hoveredOffice === office.id ? 28 : 20}
                                                fill={hoveredOffice === office.id ? "#F59E0B" : "#3B82F6"}
                                                fillOpacity="0.2"
                                                className="transition-all duration-300"
                                            />
                                            {hoveredOffice === office.id && (
                                                <>
                                                    <text x={office.mapX} y={office.mapY - 18} textAnchor="middle" fill="white" fontSize="10" fontWeight="bold">
                                                        {office.city}
                                                    </text>
                                                    <circle cx={office.mapX} cy={office.mapY} r="22" fill="none" stroke="#F59E0B" strokeWidth="2" className="animate-ping-slow" />
                                                </>
                                            )}
                                        </g>
                                    ))}
                                ...
                                return (
                                    ...
                                    >
                                        <div className="relative h-40 overflow-hidden">
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleSaveOffice(office.id); }}
                ...
                {activeTab === 'saved' && (
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                        {savedOffices.length > 0 ? (
                            config?.offices?.filter(o => savedOffices.includes(o.id)).map((office) => {
                                const regionConfig = getRegionConfig(office.region);
                                return (
                                    <div
                                        key={office.id}
                                    ...
                                    >
                                        ...
                                        <div className="p-5">
                                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{office.city}</h3>
                            ...
                            <div className="col-span-full text-center py-12">
                                ...
                                <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No saved offices</h3>
                                <p className="text-gray-500 dark:text-gray-400">Click the bookmark icon on any office to save it here.</p>
                                <button onClick={() => setActiveTab('offices')} className="mt-4 text-blue-600 hover:underline">
                                    Browse All Offices
                                </button>
                            </div>
                        )}
                ...
                {showOfficeModal && selectedOffice && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={() => setShowOfficeModal(false)}>
                        <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
                            <div className="relative h-48 overflow-hidden">
                                <img src={selectedOffice.image} alt={selectedOffice.city} className="w-full h-full object-cover" />
                                <button onClick={() => setShowOfficeModal(false)} className="absolute top-4 right-4 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white hover:bg-black/70">
                                    <HiOutlineX className="w-5 h-5" />
                                </button>
                                <div className="absolute bottom-4 left-4">
                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getRegionConfig(selectedOffice.region).color}`}>
                            ...
                            <div className="p-6">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{selectedOffice.city}</h2>
                                    {selectedOffice.isRegionalHub && (
                                        <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs rounded-full">Regional HQ</span>
                                    )}
                                </div>
                                <p className="text-gray-500 text-sm mb-4">{selectedOffice.country}</p>
                                ...
                                {selectedOffice.services && selectedOffice.services.length > 0 && (
                                    <div className="mb-4">
                                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Services Offered</p>
                                ...
                                <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg transition-colors ${savedOffices.includes(selectedOffice.id) ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-600'}`}
                                    >
