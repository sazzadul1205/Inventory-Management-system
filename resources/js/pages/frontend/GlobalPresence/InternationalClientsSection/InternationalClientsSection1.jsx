...
const InternationalClientsSection1 = ({ config }) => {
    ...
    const getIndustryConfig = (industry) => {
        const configs = {
            'retail': { color: 'bg-blue-100 text-blue-700', label: 'Retail', icon: 'building' },
            'manufacturing': { color: 'bg-purple-100 text-purple-700', label: 'Manufacturing', icon: 'cog' },
            'healthcare': { color: 'bg-green-100 text-green-700', label: 'Healthcare', icon: 'shield' },
            'logistics': { color: 'bg-orange-100 text-orange-700', label: 'Logistics', icon: 'globe' },
            'automotive': { color: 'bg-red-100 text-red-700', label: 'Automotive', icon: 'bolt' },
            'consumer-goods': { color: 'bg-indigo-100 text-indigo-700', label: 'Consumer Goods', icon: 'gift' }
        };
    ...
    const handleSaveClient = (clientId) => {
        if (savedClients.includes(clientId)) {
            setSavedClients(savedClients.filter(id => id !== clientId));
        } else {
            setSavedClients([...savedClients, clientId]);
        }
    };
    ...
    // Featured client
    const featuredClient = config?.featuredClient || filteredClients[0];
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getRegionConfig(featuredClient.region).color}`}>
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            {featuredClient.logo && (
                                                <img src={featuredClient.logo} alt={featuredClient.name} className="h-12 w-auto object-contain" />
                                            )}
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {featuredClient.testimonial && (
                                                <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                                                    ...
                                                    <p className="text-sm italic text-gray-600 dark:text-gray-400">"{featuredClient.testimonial.text}"</p>
                                                    <p className="text-xs text-gray-500 mt-2">— {featuredClient.testimonial.author}, {featuredClient.testimonial.title}</p>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-4 mb-6">
                                            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                                                ...
                                                <span>{featuredClient.city}, {featuredClient.country}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="mt-12 bg-linear-to-r from-blue-600 to-purple-600 dark:from-blue-500 dark:to-purple-500 rounded-3xl p-8 text-white text-center">
                    ...
                    <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
                        {config?.ctaDescription || "See how SupplyChainPro can transform your supply chain operations. Join hundreds of leading companies worldwide."}
                    </p>
                    <Link
                        href="/contact"
