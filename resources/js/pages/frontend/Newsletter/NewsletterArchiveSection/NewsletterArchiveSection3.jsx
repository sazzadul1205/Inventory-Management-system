...
/**
 * Newsletter Archive Section III - Full Content Hub with Carousel & Modal Reader
 *
 * Unique Design Elements:
 * - Stats Cards for Newsletter Metrics (Newsletters, Views, Satisfaction, Topics)
 * - Multi-tab UI (All Issues, Featured, Popular, Saved, Liked) with Count Badges
 * - Featured Issues Carousel with Auto-play and Manual Navigation
 * - Search Functionality with Category, Year, Month Filters
 * - Expandable Filters Panel with Active Filter Indicators
 * - Grid Layout with Interactive Cards and Hover Effects
 * - Video Thumbnail Overlay on Hover
 * - Like and Save with localStorage Persistence
 * - Full-Screen Modal Reader with Video Summary Option
 * - Video Modal for Educational Content Playback
 * - Circuit Board Background Pattern
 * - Animated Pulse Badge in Header
 * - Responsive Grid Layout for Newsletter Cards
 * - Subscribe CTA with Gradient Background
 *
...
// React Icons - Heroicons, Heroicons 2, Material Design
import {
  HiOutlineNewspaper,
  HiOutlineCalendar,
  HiOutlineClock,
...
const NewsletterArchiveSection3 = ({ config }) => {
  ...
  const featuredIssues = config?.featuredIssues || newsletterIssues.filter(issue => issue.featured);
   ...
   * Supports Heroicons, Heroicons 2, and Material Design sets
   */
  ...
  // ==================== CAROUSEL NAVIGATION ====================
  const storiesCount = featuredIssues.length;
  ...
  // Auto-play carousel
  useEffect(() => {
    if (config?.autoPlayCarousel && storiesCount > 1 && activeTab === 'all') {
  ...
  const getFilteredIssues = useCallback(() => {
    ...
    if (searchQuery) {
      ...
      issues = issues.filter(issue =>
        ...
        issue.keyTopics.some(topic => topic.toLowerCase().includes(query))
      );
    }

    if (selectedYear !== 'all') {
      issues = issues.filter(issue => issue.date.includes(selectedYear));
    }
    ...
    if (selectedMonth !== 'all') {
      ...
      issues = issues.filter(issue => issue.dateRaw.substring(5, 7) === monthNum);
    }

    if (selectedCategory !== 'all') {
      issues = issues.filter(issue => issue.category === selectedCategory);
    }

    if (activeTab === 'featured') {
      issues = issues.filter(issue => issue.featured);
    } else if (activeTab === 'popular') {
      issues = issues.filter(issue => issue.popular);
    } else if (activeTab === 'saved') {
      issues = issues.filter(issue => savedIssues.includes(issue.id));
    } else if (activeTab === 'liked') {
      issues = issues.filter(issue => likedIssues.includes(issue.id));
    }
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-4xl mx-auto mb-12">
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-gray-900 dark:text-white mb-6">
            {config?.title?.prefix || "Browse Our"}{' '}
            <span className="bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              {config?.title?.highlight || "Past Issues"}
            </span>
          </h1>

          <p className="text-xl text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
            {config?.description ||
              "Explore our complete archive of newsletters covering supply chain trends, technology insights, and industry best practices."}
          </p>
            ...
            <div className="relative overflow-hidden rounded-3xl">
              ...
              >
                {featuredIssues.map((issue, idx) => (
                  <div key={idx} className="w-full shrink-0 cursor-pointer" onClick={() => openReaderModal(issue)} role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && openReaderModal(issue)}>
                    <div className="relative h-96 rounded-3xl overflow-hidden">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                        <div className="flex flex-wrap gap-2 mb-3">
                          ...
                          <span className="px-3 py-1 rounded-full text-xs font-semibold bg-amber-500">
                            Featured
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            aria-label={likedIssues.includes(issue.id) ? "Unlike this issue" : "Like this issue"}
                          >
        ...
        <div className="mb-6">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Showing{' '}
            <span className="font-semibold text-gray-900 dark:text-white">{filteredIssues.length}</span>{' '}
            newsletter issues
          ...
          {filteredIssues.map((issue) => {
            ...
            return (
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && openReaderModal(issue)}
              >
                ...
                <div className="p-5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 font-medium mb-3"
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        aria-label={isSaved ? "Remove from saved" : "Save this issue"}
                      >
        ...
        {showReaderModal && selectedIssue && (
          <div
            ...
            role="dialog"
            aria-label={`${selectedIssue.title} - full newsletter reader`}
            aria-modal="true"
          >
            <div
              className="relative max-w-3xl w-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl my-8"
            ...
            >
              ...
              <div className="p-6">
                ...
                {selectedIssue.videoUrl && (
                  <button
                    onClick={() => {
                      openVideoModal(selectedIssue.videoUrl);
                      closeReaderModal();
                    }}
                    className="w-full mb-4 inline-flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-2 rounded-lg font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                ...
                <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                  <a
                    href={`/newsletter/${selectedIssue.id}/download`}
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-colors ${savedIssues.includes(selectedIssue.id) ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
                      }`}
                    aria-label={savedIssues.includes(selectedIssue.id) ? "Remove from saved" : "Save this issue"}
                  >
