...
const SubscribeFormSection1 = ({ config }) => {
    ...
    return (
        <section
            ...
            role="region"
            aria-label="Newsletter Subscribe Section"
        >
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-12">
                    ...
                    <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6">
                        {config?.title?.prefix || "Subscribe to Our"} <span className="bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">{config?.title?.highlight || "Newsletter"}</span> {config?.title?.suffix || ""}
                    </h2>
                                ...
                                {stat.icon === 'users' ? <HiOutlineUserGroup className="w-5 h-5 text-blue-600" /> :
                                    stat.icon === 'eye' ? <HiOutlineEye className="w-5 h-5 text-blue-600" /> :
                                        stat.icon === 'calendar' ? <HiOutlineCalendar className="w-5 h-5 text-blue-600" /> :
                                            <HiOutlineGift className="w-5 h-5 text-blue-600" />}
                            </div>
                            <div>
                    ...
                    {benefits.map((benefit, idx) => (
                        <div key={idx} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 text-center">
                            <div className="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center mx-auto mb-4">
                                {benefit.icon === 'newspaper' ? <HiOutlineNewspaper className="w-6 h-6 text-blue-600" /> :
                                    benefit.icon === 'academic' ? <HiOutlineAcademicCap className="w-6 h-6 text-blue-600" /> :
                                        benefit.icon === 'chip' ? <HiOutlineChip className="w-6 h-6 text-blue-600" /> :
