...
const PageHero = ({ heroData }) => {
  ...
  const title = heroData?.title || 'Client';
  const highlightedText = heroData?.highlightedText || 'Success Stories';
  const description = heroData?.description || 'Discover how our innovative solutions have transformed businesses across industries, delivering measurable results and exceptional ROI.';
  ...
  return (
    <section
      className="relative bg-linear-to-r from-green-600 to-emerald-700 dark:from-green-900 dark:to-emerald-900 text-white overflow-hidden"
      aria-label="Success stories page hero section"
    >
      ...
      <div className="container mx-auto px-4 py-20 md:py-28 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" className="flex items-center justify-center gap-2 text-sm text-green-100 dark:text-green-200 mb-6">
            <a href="/" className="hover:text-white dark:hover:text-green-100 transition-colors">Home</a>
            <span aria-hidden="true">•</span>
            <span className="text-white dark:text-white" aria-current="page">Success Stories</span>
          </nav>
          ...
          {/* Description */}
          <p className="text-lg md:text-xl text-green-100 dark:text-green-200 mb-8 max-w-2xl mx-auto">
            {description}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
            <button
              className="px-8 py-3 bg-yellow-400 dark:bg-yellow-500 text-green-900 dark:text-green-950 font-semibold rounded-lg hover:bg-yellow-300 dark:hover:bg-yellow-400 transition-colors transform hover:scale-105 duration-200"
