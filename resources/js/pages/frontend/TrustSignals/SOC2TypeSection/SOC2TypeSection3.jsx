...
/**
 * SOC 2 Type II Section III - Full Compliance Hub with Stories & Video
 *
 * Unique Design Elements:
 * - Multi-tab UI (Trust Services, Customer Stories, Request Report)
 * - Stats Cards for SOC 2 Metrics (Type II, Uptime SLA, Monitoring, Audit Cycle)
 * - Trust Service Criteria Cards with Expandable Controls and Metrics
 * - Video Explanations for Trust Services with Modal Player
 * - Customer Stories Carousel with Auto-play and Manual Navigation
 * - Multi-step Request Form with Application ID Generation
 * - Success Message with Reference ID on Form Submission
...
const SOC2TypeSection3 = ({ config }) => {
  ...
  const generateApplicationId = () => {
    return `SOC2-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
  };
        ...
        {activeTab === 'overview' && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {trustServices.map((service, idx) => (
              <div
                key={idx}
                className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-200 dark:border-gray-700 cursor-pointer group"
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-emerald-600 dark:text-emerald-400">{service.metrics}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleTrustService(idx);
                    }}
                    className="flex items-center gap-1 text-sm text-blue-600 dark:text-blue-400 font-medium"
                    aria-label={activeTrustService === idx ? 'Hide controls' : `View ${service.controls.length} controls`}
                  >
                    {activeTrustService === idx ? 'Hide controls' : `View ${service.controls.length} controls`}
                    <HiOutlineChevronDown
                      className={`w-4 h-4 transition-transform ${activeTrustService === idx ? 'rotate-180' : ''}`}
                    />
                  </button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-xl border border-gray-200 dark:border-gray-700">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Complete the form below to request access to our SOC 2 Type II report. We'll verify your request and
                provide access within 2 business days.
              </p>
            </div>
            ...
            {formSubmitted ? (
              <div className="text-center py-12 animate-fadeIn">
                ...
                <p className="text-gray-500 text-sm">
                  Our security team will review and provide access to the SOC 2 Type II report within 2 business days.
                </p>
              </div>
            ) : (
