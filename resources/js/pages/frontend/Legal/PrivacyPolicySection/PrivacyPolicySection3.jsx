// page/frontend/Legal/PrivacyPolicySection/PrivacyPolicySection3.jsx

/**
 * Privacy Policy Section III - AI-Powered Legal Documentation Hub
 *
 * Unique Design Elements:
 * - Policy Highlights Carousel with Video Demos
 * - Multi-Tab Interface (Full Policy, Highlights, FAQ, Your Rights)
 * - Video Tutorial Integration for Key Concepts
 * - User Rights Summary Cards
 * - Customer Testimonials Section
 * - Expandable FAQ with Video Explanations
...
const PrivacyPolicySection3 = ({ config }) => {
  ...
  const nextSlide = useCallback(() => {
    setCurrentSlide((prev) => (prev + 1) % policyHighlights.length);
  }, [policyHighlights.length]);

  const prevSlide = useCallback(() => {
    setCurrentSlide((prev) => (prev - 1 + policyHighlights.length) % policyHighlights.length);
  }, [policyHighlights.length]);

  // ==================== AUTO-PLAY CAROUSEL EFFECT ====================
  useEffect(() => {
    if (config?.autoPlayCarousel !== false && policyHighlights.length > 1) {
      const interval = setInterval(() => {
        nextSlide();
      }, 6000);
      return () => clearInterval(interval);
    }
  }, [config?.autoPlayCarousel, nextSlide, policyHighlights.length]);
    ...
    if (section.id === 'information-collection') {
      return (
        <div className="space-y-4">
            ...
            {content.categories.map((cat, idx) => (
              <div key={idx} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  {getIcon(cat.icon, "w-5 h-5 text-blue-600 dark:text-blue-400")}
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 italic">{content.legalBasis}</p>
        </div>
      );
    }
            ...
            {content.measures.map((measure, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div>
                  ...
                  <span className={`inline-block text-xs mt-1 px-2 py-0.5 rounded-full ${measure.status === 'Active' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300'}`}>
                    {measure.status}
                  </span>
                </div>
              </div>
    ...
    if (section.id === 'user-rights') {
      return (
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  <th className="text-left p-3 font-semibold text-gray-900 dark:text-white">Right</th>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 italic">{content.howToExercise}</p>
        </div>
      );
    }
    ...
    if (section.id === 'cookies') {
      return (
        <div className="space-y-4">
                  ...
                  <tr key={idx} className="border-b border-gray-100 dark:border-gray-700">
                    ...
                    <td className="p-3">
                      {cookie.required ? (
                        ...
                        <span className="text-gray-500 dark:text-gray-400">Opt-in</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{content.controlOptions}</p>
        </div>
      );
    }
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-2">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              <strong>Email:</strong> <a href={`mailto:${content.company.email}`} className="text-blue-600 dark:text-blue-400 hover:underline">{content.company.email}</a>
            </p>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{content.escalationPath}</p>
        </div>
      );
    }
            ...
            {quickFacts.map((fact, idx) => (
              <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-5 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <div className="text-left">
                  ...
                  {fact.trend && (
                    <div className="text-xs text-green-500 dark:text-green-400">{fact.trend}</div>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-xl border border-gray-200 dark:border-gray-700">
            ...
            <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-center">
              <p className="text-gray-600 dark:text-gray-400">
                ...
                <a href="mailto:privacy@supplychainpro.com" className="text-blue-600 dark:text-blue-400 font-medium hover:underline">
          ...
          <div className="space-y-8">
              ...
              {sections.find(s => s.id === 'user-rights')?.content.rights.map((right, idx) => (
                <div key={idx} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow">
                  <div className="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center mb-4">
                    <HiOutlineCheckCircle className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 dark:text-gray-400">Response: {right.timeframe}</span>
                    <span className="text-blue-600 dark:text-blue-400">{right.method}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-linear-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 text-white text-center">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <button className="inline-flex items-center gap-2 bg-white text-blue-600 px-6 py-3 rounded-xl font-semibold hover:bg-blue-50 transition-all duration-300" aria-label="Contact privacy team">
                  <HiOutlineMail className="w-5 h-5" />
                  Contact Privacy Team
                </button>
                <button className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-6 py-3 rounded-xl font-semibold hover:bg-white/30 transition-all duration-300" aria-label="Request data export">
