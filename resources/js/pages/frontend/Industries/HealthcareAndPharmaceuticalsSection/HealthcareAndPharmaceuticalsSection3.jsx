// page/frontend/Industries/HealthcareAndPharmaceuticals/HealthcareAndPharmaceuticalsSection3.jsx
...
const HealthcareAndPharmaceuticalsSection3 = ({ config }) => {
  ...
  const newLocal = "w-4 h-4 text-green-500 mt-0.5 flex-shrink-0";
  return (
    <section
    ...
    >
      ...
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-teal-100 dark:bg-teal-900/10 rounded-full filter blur-3xl" aria-hidden="true"></div>
      <div className="absolute bottom-0 right-0 w-80 h-80 bg-emerald-100 dark:bg-emerald-900/10 rounded-full filter blur-3xl" aria-hidden="true"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {benefits.map((benefit, index) => (
              <div key={index} className="text-center p-6 bg-gray-50 dark:bg-gray-800/50 rounded-2xl hover:shadow-lg transition-all">
                ...
                <div className="text-xl font-bold text-teal-600 dark:text-teal-400 mb-2">{benefit.value}</div>
              ...
              {features[selectedFeature] && (
                <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-8">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-teal-600 font-semibold hover:gap-3 transition-all mt-4"
                  >
          ...
          <div className="mb-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-8">
                <div className="text-center mb-4">
                  <div className="text-3xl mb-2">😊</div>
                  <h3 className="text-xl font-bold text-teal-600">After Our Solution</h3>
                </div>
                <ul className="space-y-3">
                  {config?.comparison?.after?.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-gray-600 dark:text-gray-400">
                      <HiOutlineCheckCircle className={newLocal} />
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              {config?.steps?.map((step, index) => (
                <div key={index} className="text-center relative">
                  {index < config?.steps?.length - 1 && (
                    <div className="hidden md:block absolute top-12 left-1/2 w-full h-0.5 bg-linear-to-r from-teal-200 to-emerald-200 dark:from-teal-800 dark:to-emerald-800" aria-hidden="true"></div>
                  )}
                  <div className="relative z-10 w-16 h-16 bg-teal-100 dark:bg-teal-900/30 rounded-full flex items-center justify-center text-2xl font-bold text-teal-600 mx-auto mb-4">
                    {index + 1}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {config?.resources?.map((resource, index) => (
                ...
                >
                  ...
                  <h4 className="font-bold text-gray-900 dark:text-white mb-2 group-hover:text-teal-600 transition-colors">
                    {resource.title}
                  </h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{resource.description}</p>
                  <span className="text-sm text-teal-600 font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                    {resource.cta}
