/**
 * Electronics Section 1 Component
 * Showcases electronics industry solutions with:
 * - Statistics display for electronics metrics
 * - Tabbed content (Overview, Challenges, Solutions)
 * - Interactive feature highlights
 * - Industry standards badges (ISO, IPC, RoHS, etc.)
...
const ElectronicsSection1 = ({ config }) => {
  // ==================== STATE MANAGEMENT ====================
  ...
  const getTabs = useCallback(() => {
    return [
      { id: 'overview', label: 'Overview', icon: 'chip' },
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Electronics Solutions Section"
    >
      {/* ==================== BACKGROUND DECORATIONS ==================== */}
      <div className="absolute inset-0 bg-noise-pattern opacity-5 dark:opacity-10" aria-hidden="true" />
      <div className="absolute top-0 right-0 w-1/3 h-full bg-linear-to-l from-purple-50/30 to-transparent dark:from-purple-900/5 pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-0 left-0 w-1/3 h-full bg-linear-to-r from-indigo-50/30 to-transparent dark:from-indigo-900/5 pointer-events-none" aria-hidden="true" />

      {/* Floating decorative elements */}
      <div className="absolute top-20 right-10 w-32 h-32 bg-purple-300/5 dark:bg-purple-500/5 rounded-full blur-3xl" aria-hidden="true" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {activeTab === 'overview' && config?.overview && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center animate-fadeIn">
              ...
              <div>
                ...
                <div className="mt-8">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-semibold hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                  >
              ...
              <div className="relative">
                <div className="bg-linear-to-br from-purple-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl p-8 border border-purple-100 dark:border-gray-700">
                  <div className="flex justify-center mb-5">
                    {getIcon("chip", "w-12 h-12 text-purple-600")}
                  </div>
          ...
          {activeTab === 'challenges' && challenges.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
              {challenges.map((challenge, index) => (
                ...
                >
                  <div className="flex justify-center mb-3">
                    {getIcon(challenge.icon, "w-10 h-10 text-purple-600")}
                  </div>
          ...
          {activeTab === 'solutions' && solutions.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
              {solutions.map((solution, index) => (
                <div
                  key={index}
                  className="p-6 bg-linear-to-br from-purple-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-purple-100 dark:border-gray-700 group"
                >
                  <div className="flex justify-center mb-4">
                    {getIcon(solution.icon, "w-10 h-10 text-purple-600")}
                  </div>
                  <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2 text-center">
                    {solution.title}
                  </h4>
                  ...
                  {solution.features && solution.features.length > 0 && (
                    <ul className="space-y-1.5 mt-3 pt-3 border-t border-purple-100 dark:border-gray-700">
        ...
        {config?.showFeatures && features?.items?.length > 0 && (
          <div className="mb-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-8 hover:shadow-lg transition-all">
            <h3 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white text-center mb-6">
              {features.title || "Why Electronics Leaders Choose Us"}
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {features.items.map((feature, index) => (
                <div key={index} className="text-center group">
                  <div className="flex justify-center mb-2">
                    {getIcon(feature.icon, "w-8 h-8 text-purple-600 group-hover:scale-110 transition-transform")}
                  </div>
        ...
        {config?.showStandards && (
          <div className="mb-12 text-center">
            <div className="inline-flex flex-wrap items-center justify-center gap-3 px-6 py-3 bg-purple-50 dark:bg-purple-900/20 rounded-full">
              {getIcon("shield", "w-5 h-5 text-purple-600")}
              <span className="text-sm text-gray-700 dark:text-gray-300">
                {config?.standardsText || "ISO 9001 | IPC-A-610 | J-STD-001 | RoHS Compliant | REACH | UL Certified"}
              </span>
            </div>
          </div>
        )}
        ...
        {config?.showSuccessMetrics && config?.successMetrics && (
          <div className="mb-12 bg-linear-to-r from-purple-600 to-indigo-600 rounded-2xl p-6 text-white shadow-xl">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 text-center">
              {config.successMetrics.map((metric, index) => (
                <div key={index} className="animate-fadeIn" style={{ animationDelay: `${index * 100}ms` }}>
                  ...
                  <div className="text-sm text-purple-100 mt-1">{metric.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-5 p-6 bg-linear-to-r from-purple-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-purple-100 dark:border-gray-700">
              ...
              <span className="text-gray-700 dark:text-gray-300 font-medium text-center sm:text-left">
                {config?.ctaText || "Ready to transform your electronics operations?"}
              </span>
              <Link
...
export default ElectronicsSection1;