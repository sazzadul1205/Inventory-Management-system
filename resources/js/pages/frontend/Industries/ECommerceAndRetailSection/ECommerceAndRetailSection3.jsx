// pages/frontend/Industries/ECommerceAndRetailSection/ECommerceAndRetailSection3.jsx

/**
 * E-Commerce & Retail Section 3 Component - Features & FAQ
 * Comprehensive solution showcase for retail and e-commerce businesses featuring:
...
const ECommerceAndRetailSection3 = ({ config }) => {
  // ==================== STATE MANAGEMENT ====================
  ...
  return (
    <section
      ...
      role="region"
      aria-label="E-Commerce & Retail Features Section"
    >
      {/* ==================== BACKGROUND DECORATIONS ==================== */}
      <div className="absolute inset-0 bg-noise-pattern opacity-5 dark:opacity-10" aria-hidden="true" />
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-orange-100 dark:bg-orange-900/10 rounded-full filter blur-3xl" aria-hidden="true" />
      <div className="absolute bottom-0 right-0 w-80 h-80 bg-amber-100 dark:bg-amber-900/10 rounded-full filter blur-3xl" aria-hidden="true" />

      {/* Floating decorative elements */}
      <div className="absolute top-32 right-10 w-24 h-24 bg-orange-300/5 dark:bg-orange-500/5 rounded-full blur-2xl" aria-hidden="true" />
      <div className="absolute bottom-32 left-10 w-32 h-32 bg-amber-300/5 dark:bg-amber-500/5 rounded-full blur-2xl" aria-hidden="true" />
            ...
            {benefits.map((benefit, index) => (
              ...
              >
                <div className="flex justify-center mb-3">
                  {getIcon(benefit.icon, "w-8 h-8 text-orange-600")}
                </div>
                <div className="text-xl md:text-2xl font-bold text-orange-600 dark:text-orange-400 mb-1 font-mono">
                  {benefit.value}
                </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="shrink-0">
                        {getIcon(feature.icon, "w-6 h-6 text-orange-600")}
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 md:p-8 transition-all duration-300 animate-fadeIn">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 rounded-xl flex items-center justify-center">
                        {getIcon(currentFeature.icon, "w-6 h-6 text-orange-600")}
                      </div>
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-2 text-orange-600 dark:text-orange-400 font-semibold hover:gap-3 transition-all group"
                    >
          ...
          <div className="mb-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl overflow-hidden shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-6 md:p-8">
                <div className="text-center mb-5">
                  <div className="flex justify-center mb-3">
                    {getIcon("sparkles", "w-10 h-10 text-orange-500")}
                  </div>
                  <h3 className="text-xl font-bold text-orange-600 dark:text-orange-400">After Our Solution</h3>
                </div>
                <ul className="space-y-3">
        ...
        {config?.showTechStack && config?.techStack?.length > 0 && (
          <div className="mb-12">
            <h3 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white text-center mb-8">
              {config?.techStackTitle || "Seamlessly Integrates With Your Tech Stack"}
            </h3>
              ...
              {config.steps.map((step, index) => (
                <div key={index} className="text-center relative">
                  ...
                  {index < config.steps.length - 1 && (
                    <div className="hidden md:block absolute top-12 left-1/2 w-full h-0.5 bg-linear-to-r from-orange-200 to-amber-200 dark:from-orange-800 dark:to-amber-800" aria-hidden="true" />
                  )}

                  {/* Step Number */}
                  <div className="relative z-10 w-12 h-12 bg-orange-100 dark:bg-orange-900/30 rounded-full flex items-center justify-center text-lg font-bold text-orange-600 mx-auto mb-4">
                    {index + 1}
                  </div>

                  {/* Step Icon */}
                  <div className="flex justify-center mb-3">
                    {getIcon(step.icon, "w-8 h-8 text-orange-600")}
                  </div>
              ...
              {faqs.map((faq, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-orange-500 text-xl shrink-0">
                      {expandedFaq === index ? (
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {config.resources.map((resource, index) => (
                ...
                >
                  <div className="flex justify-center mb-3">
                    {getIcon(resource.icon, "w-10 h-10 text-orange-600")}
                  </div>
                  <h4 className="font-bold text-gray-900 dark:text-white mb-2 text-center group-hover:text-orange-600 transition-colors">
                    {resource.title}
                  </h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-3 leading-relaxed">
                    {resource.description}
                  </p>
                  <span className="text-sm text-orange-600 dark:text-orange-400 font-semibold inline-flex items-center justify-center gap-1 w-full group-hover:gap-2 transition-all">
                    {resource.cta}
          ...
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-5 p-6 bg-linear-to-r from-orange-50 to-amber-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-orange-100 dark:border-gray-700">
              <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 rounded-full flex items-center justify-center">
                {getIcon("bag", "w-6 h-6 text-orange-600")}
              </div>
              <span className="text-gray-700 dark:text-gray-300 font-medium text-center sm:text-left">
                {config?.ctaText || "Ready to transform your retail operations?"}
              </span>
              <Link
...
export default ECommerceAndRetailSection3;