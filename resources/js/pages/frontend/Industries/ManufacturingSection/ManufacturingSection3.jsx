// page/frontend/Industries/ManufacturingSection/ManufacturingSection3.jsx

/**
 * Manufacturing Section 3 Component - Features & FAQ
 * Comprehensive manufacturing solution showcase featuring:
...
const ManufacturingSection3 = ({ config }) => {
  // ==================== STATE MANAGEMENT ====================
  ...
  const getIcon = useCallback((iconName, className = "w-5 h-5") => {
    ...
    const IconComponent = icons[iconName] || MdFactory;
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Manufacturing Features Section"
    >
      ...
      <div className="absolute bottom-0 right-0 w-80 h-80 bg-cyan-100 dark:bg-cyan-900/10 rounded-full filter blur-3xl" aria-hidden="true" />
      ...
      <div className="absolute bottom-32 left-10 w-32 h-32 bg-cyan-300/5 dark:bg-cyan-500/5 rounded-full blur-2xl" aria-hidden="true" />
            ...
            {benefits.map((benefit, index) => (
              ...
              >
                <div className="flex justify-center mb-3">
                  {getIcon(benefit.icon, "w-8 h-8 text-blue-600")}
                </div>
                <div className="text-xl md:text-2xl font-bold text-blue-600 dark:text-blue-400 mb-1 font-mono">
                  {benefit.value}
                </div>
                ...
                {features.map((feature, index) => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl transition-all duration-200 group ${selectedFeature === index
                      ? 'bg-linear-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border-l-4 border-blue-500 shadow-md'
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="shrink-0">
                        {getIcon(feature.icon, "w-6 h-6 text-blue-600")}
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 md:p-8 transition-all duration-300 animate-fadeIn">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center">
                        {getIcon(currentFeature.icon, "w-6 h-6 text-blue-600")}
                      </div>
          ...
          <div className="mb-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl overflow-hidden shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-6 md:p-8">
                <div className="text-center mb-5">
                  <div className="flex justify-center mb-3">
                    {getIcon("sparkles", "w-10 h-10 text-blue-500")}
                  </div>
                  <h3 className="text-xl font-bold text-blue-600 dark:text-blue-400">After Our Solution</h3>
                </div>
                <ul className="space-y-3">
        ...
        {config?.showTechStack && config?.techStack?.length > 0 && (
          <div className="mb-12">
            <h3 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white text-center mb-8">
              {config?.techStackTitle || "Seamlessly Integrates With Your Manufacturing Stack"}
            </h3>
              ...
              {config.steps.map((step, index) => (
                <div key={index} className="text-center relative">
                  ...
                  {index < config.steps.length - 1 && (
                    <div className="hidden md:block absolute top-12 left-1/2 w-full h-0.5 bg-linear-to-r from-blue-200 to-cyan-200 dark:from-blue-800 dark:to-cyan-800" aria-hidden="true" />
                  )}

                  {/* Step Number */}
                  <div className="relative z-10 w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-lg font-bold text-blue-600 mx-auto mb-4">
                    {index + 1}
                  </div>

                  {/* Step Icon */}
                  <div className="flex justify-center mb-3">
                    {getIcon(step.icon, "w-8 h-8 text-blue-600")}
                  </div>
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {config.resources.map((resource, index) => (
                ...
                >
                  <div className="flex justify-center mb-3">
                    {getIcon(resource.icon, "w-10 h-10 text-blue-600")}
                  </div>
                  <h4 className="font-bold text-gray-900 dark:text-white mb-2 text-center group-hover:text-blue-600 transition-colors">
                    {resource.title}
                  </h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-3 leading-relaxed">
                    {resource.description}
                  </p>
                  <span className="text-sm text-blue-600 dark:text-blue-400 font-semibold inline-flex items-center justify-center gap-1 w-full group-hover:gap-2 transition-all">
                    {resource.cta}
          ...
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-5 p-6 bg-linear-to-r from-blue-50 to-cyan-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-blue-100 dark:border-gray-700">
              <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center">
                {getIcon("factory", "w-6 h-6 text-blue-600")}
              </div>
