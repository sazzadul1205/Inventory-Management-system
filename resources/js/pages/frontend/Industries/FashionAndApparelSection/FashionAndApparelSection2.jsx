/**
 * Fashion & Apparel Section 2 Component - Success Stories Carousel
 * Features fashion and apparel industry customer success stories with:
 * - Interactive carousel/slider for featured case studies
 * - Results grid display
 * - Sustainability badges (GOTS, OEKO-TEX, etc.)
 * - Testimonials with quotes
 * - Additional case studies grid
 * - Markdown optimization calculator preview
 * - Sustainable fashion partners showcase
 * 
...
const FashionAndApparelSection2 = ({ config }) => {
  // ==================== STATE MANAGEMENT ====================
  ...
  const getIcon = useCallback((iconName, className = "w-5 h-5") => {
    ...
    const IconComponent = icons[iconName] || HiOutlineShoppingBag;
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Fashion & Apparel Success Stories Section"
    >
      {/* ==================== BACKGROUND DECORATIONS ==================== */}
      <div className="absolute inset-0 bg-dots-pattern opacity-30 dark:opacity-10" aria-hidden="true" />
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-pink-200 dark:bg-pink-900/20 rounded-full filter blur-3xl" aria-hidden="true" />
      <div className="absolute bottom-1/4 left-0 w-96 h-96 bg-rose-200 dark:bg-rose-900/20 rounded-full filter blur-3xl" aria-hidden="true" />

      {/* Floating decorative elements */}
      <div className="absolute top-20 right-10 w-24 h-24 bg-pink-300/5 dark:bg-pink-500/5 rounded-full blur-2xl" aria-hidden="true" />
      <div className="absolute bottom-20 left-10 w-32 h-32 bg-rose-300/5 dark:bg-rose-500/5 rounded-full blur-2xl" aria-hidden="true" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {currentCase && (
          <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden mb-12 transition-all duration-300">
            ...
            <div className="bg-linear-to-r from-pink-600 to-rose-600 p-8 text-white">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {getIcon("location", "w-4 h-4 text-pink-200")}
                      <p className="text-pink-100 text-sm">{currentCase.location}</p>
                    </div>
                  </div>
                </div>
                <div className="text-center md:text-right">
                  <div className="text-sm text-pink-200 mb-1">Key Result</div>
                  <div className="text-3xl md:text-4xl font-bold">{currentCase.keyResult?.value}</div>
                  <div className="text-xs text-pink-200">{currentCase.keyResult?.label}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30">
              {currentCase.results?.map((result, idx) => (
                <div key={idx} className="text-center">
                  <div className="text-xl md:text-2xl font-bold text-pink-600 dark:text-pink-400 font-mono">
                    {result.value}
                  </div>
            ...
            <div className="p-6 md:p-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="animate-fadeIn">
                  <div className="flex items-center gap-2 mb-3">
                    {getIcon("chart", "w-5 h-5 text-pink-500")}
                ...
                <div className="animate-fadeIn" style={{ animationDelay: "100ms" }}>
                  <div className="flex items-center gap-2 mb-3">
                    {getIcon("bulb", "w-5 h-5 text-pink-500")}
              ...
              {currentCase.sustainability && currentCase.sustainability.length > 0 && (
                <div className="mt-5 flex flex-wrap gap-2">
                  {currentCase.sustainability.map((badge, idx) => (
                    <span key={idx} className="text-xs px-2.5 py-1 bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-400 rounded-full flex items-center gap-1">
              ...
              {currentCase.testimonial && (
                <div className="mt-6 p-6 bg-pink-50 dark:bg-pink-900/20 rounded-2xl border border-pink-100 dark:border-pink-800">
              ...
              <div className="mt-6 flex flex-col sm:flex-row gap-3">
                <Link
                  ...
                  className="flex items-center justify-center gap-2 px-5 py-2.5 bg-linear-to-r from-pink-600 to-rose-600 text-white rounded-xl font-semibold hover:shadow-md transition-all"
                >
                ...
                <Link
                  ...
                  className="flex items-center justify-center gap-2 px-5 py-2.5 border-2 border-pink-600 text-pink-600 dark:text-pink-400 rounded-xl font-semibold hover:bg-pink-50 dark:hover:bg-pink-900/20 transition-all"
                >
                ...
                >
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-3">
                      {getIcon(caseStudy.icon, "w-10 h-10 text-pink-600")}
                      <span className="text-xs px-2 py-1 bg-pink-100 dark:bg-pink-900/30 text-pink-600 rounded-full">
                        {caseStudy.industry}
                      </span>
                    </div>
                    <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{caseStudy.company}</h4>
                    <div className="mb-3">
                      <span className="text-xl font-bold text-pink-600 dark:text-pink-400">{caseStudy.result}</span>
                    ...
                    <Link
                      href={caseStudy.link}
                      className="inline-flex items-center gap-1 text-pink-600 dark:text-pink-400 font-semibold text-sm hover:gap-2 transition-all group-hover:text-pink-700"
                    >
                      Read Story
        ...
        {/* ==================== MARKDOWN SAVINGS CALCULATOR ==================== */}
        {config?.showMarkdownCalculator && (
          <div className="bg-linear-to-r from-pink-50 to-rose-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl p-6 mb-12 hover:shadow-lg transition-all">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
              <div className="lg:col-span-2">
                <div className="flex items-center gap-2 mb-2">
                  {getIcon("dollar", "w-5 h-5 text-pink-600")}
                  <h3 className="font-bold text-gray-900 dark:text-white">Calculate Your Markdown Optimization Savings</h3>
                </div>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">
                  See how much you could save by optimizing markdown timing and depth:
                </p>
                <div className="flex flex-wrap gap-6">
                  <div>
                    <div className="text-xl font-bold text-pink-600 dark:text-pink-400">{config?.markdownReduction || "20-30%"}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Markdown Reduction</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-pink-600 dark:text-pink-400">{config?.marginImprovement || "15-25%"}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Margin Improvement</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-pink-600 dark:text-pink-400">{config?.paybackPeriod || "3-6"}</div>
              ...
              <div>
                <Link
                  href={config?.calculatorLink || "/markdown-calculator"}
                  className="block text-center px-5 py-2.5 bg-linear-to-r from-pink-600 to-rose-600 text-white rounded-xl font-semibold hover:shadow-md transition-all"
                >
        ...
        {config?.showPartners && config?.partners?.length > 0 && (
          <div className="mb-12 text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              {config?.partnersTitle || "Leading sustainable fashion brands trust us"}
            </p>
        ...
        {config?.showCta && (
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-5 p-6 bg-linear-to-r from-pink-50 to-rose-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-pink-100 dark:border-gray-700">
              <div className="w-12 h-12 bg-pink-100 dark:bg-pink-900/30 rounded-full flex items-center justify-center">
                {getIcon("bag", "w-6 h-6 text-pink-600")}
              </div>
              ...
              <Link
                ...
                className="px-6 py-3 bg-linear-to-r from-pink-600 to-rose-600 text-white rounded-xl font-semibold hover:shadow-xl transition-all duration-300 transform hover:scale-105 inline-flex items-center gap-2"
              >
...
export default FashionAndApparelSection2;