...
const MobileDashboardSection1 = ({ config }) => {
    ...
    const metrics = config?.metrics || [
        { value: "2,847", label: "Total Shipments", icon: "globe", change: "+12.5%", changeUp: true, color: "from-blue-500 to-blue-600" },
        { value: "156", label: "Active Orders", icon: "clock", change: "+8.2%", changeUp: true, color: "from-green-500 to-green-600" },
        { value: "98.5%", label: "On-Time Delivery", icon: "check", change: "+2.1%", changeUp: true, color: "from-purple-500 to-purple-600" },
        { value: "$2.4M", label: "Inventory Value", icon: "database", change: "-3.2%", changeUp: false, color: "from-orange-500 to-orange-600" }
    ];
    ...
    return (
        <section
            ...
            role="region"
            aria-label="Mobile Dashboard Section"
        >
                                                ...
                                                {recentActivity.slice(0, 3).map((activity) => (
                                                    <div key={activity.id} className="flex items-center gap-2 text-xs">
                                                        <div className="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                                                            {activity.icon === 'check' ? <HiOutlineCheckCircle className="w-3 h-3 text-green-500" /> :
                                                             activity.icon === 'refresh' ? <HiOutlineRefresh className="w-3 h-3 text-blue-500" /> :
                                                             activity.icon === 'bell' ? <HiOutlineBell className="w-3 h-3 text-yellow-500" /> :
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {metrics.map((metric, idx) => (
                            <div
                                ...
                                onClick={() => setActiveMetric(activeMetric === idx ? -1 : idx)}
                            >
                                ...
                                {activeMetric === idx && (
                                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                        <p className="text-xs text-gray-500">Tap to view detailed analytics and trends for this metric.</p>
                                    </div>
                                )}
                            </div>
                        ))}
                        ...
                        {recentActivity.map((activity) => (
                            <div key={activity.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                                    {activity.icon === 'check' ? <HiOutlineCheckCircle className="w-5 h-5 text-green-500" /> :
                                     activity.icon === 'refresh' ? <HiOutlineRefresh className="w-5 h-5 text-blue-500" /> :
                                     activity.icon === 'bell' ? <HiOutlineBell className="w-5 h-5 text-yellow-500" /> :
                                ...
                                <div className="text-xs">
                                    {activity.status === 'completed' && <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full">Completed</span>}
                                    {activity.status === 'updated' && <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full">Updated</span>}
                                    {activity.status === 'pending' && <span className="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full">Pending</span>}
                                    {activity.status === 'in-progress' && <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded-full">In Transit</span>}
                                    {activity.status === 'alert' && <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full">Alert</span>}
                                </div>
                            </div>
                        ))}
                    </div>
