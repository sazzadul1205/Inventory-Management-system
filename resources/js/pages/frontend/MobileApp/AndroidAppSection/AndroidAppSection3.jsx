...
const AndroidAppSection3 = ({ config }) => {
  ...
  const screenshots = config?.screenshots || [
    { src: "/android-screenshots/dashboard.png", title: "Dashboard", description: "View key metrics at a glance" },
    { src: "/android-screenshots/tracking.png", title: "Live Tracking", description: "Track shipments in real-time" },
    { src: "/android-screenshots/scanning.png", title: "Barcode Scanner", description: "Scan products instantly" },
    { src: "/android-screenshots/analytics.png", title: "Analytics", description: "Deep dive into your data" },
    { src: "/android-screenshots/collaboration.png", title: "Team Chat", description: "Collaborate with your team" },
    { src: "/android-screenshots/settings.png", title: "Settings", description: "Customize your experience" }
  ];
            ...
            {stats.map((stat, idx) => (
              <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-5 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
                <div className="w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                  {stat.icon === 'star' ? <HiOutlineStar className="w-4 h-4 text-green-600" /> :
                    stat.icon === 'download' ? <HiOutlineDownload className="w-4 h-4 text-green-600" /> :
                      stat.icon === 'clock' ? <HiOutlineClock className="w-4 h-4 text-green-600" /> :
                        <HiOutlineChat className="w-4 h-4 text-green-600" />}
                </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        {feature.videoUrl && (
                          <button
                            ...
                            className="mt-3 inline-flex items-center gap-2 text-green-600 text-sm font-semibold hover:underline"
                          >
                ...
                <>
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                    {screenshots.map((_, idx) => (
                      <button key={idx} onClick={() => setCurrentSlide(idx)} className={`w-2 h-2 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-6 bg-green-600' : 'bg-gray-400'}`} />
                    ))}
                  </div>
                </>
              )}
