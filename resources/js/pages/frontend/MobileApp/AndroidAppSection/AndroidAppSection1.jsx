...
const AndroidAppSection1 = ({ config }) => {
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Android App Section"
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6">
            {config?.title?.prefix || "SupplyChainPro for"} <span className="bg-linear-to-r from-green-600 to-teal-600 bg-clip-text text-transparent">{config?.title?.highlight || "Android"}</span> {config?.title?.suffix || ""}
          </h2>
          ...
          {stats.map((stat, idx) => (
            <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-6 py-3 shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                {stat.icon === 'star' ? <HiOutlineStar className="w-5 h-5 text-green-600" /> :
                  stat.icon === 'download' ? <HiOutlineDownload className="w-5 h-5 text-green-600" /> :
                    stat.icon === 'clock' ? <HiOutlineClock className="w-5 h-5 text-green-600" /> :
          ...
          {features.map((feature, idx) => (
            <div key={idx} className="flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-300">
              <div className="w-10 h-10 rounded-xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center shrink-0">
                {feature.icon === 'globe' ? <HiOutlineGlobe className="w-5 h-5 text-green-600" /> :
                  feature.icon === 'wifi' ? <HiOutlineWifi className="w-5 h-5 text-green-600" /> :
                    feature.icon === 'qrcode' ? <HiOutlineQrcode className="w-5 h-5 text-green-600" /> :
                      feature.icon === 'bell' ? <HiOutlineBell className="w-5 h-5 text-green-600" /> :
                        feature.icon === 'shield' ? <HiOutlineShieldCheck className="w-5 h-5 text-green-600" /> :
            ...
            {compatibility.map((item, idx) => (
              <div key={idx} className="text-center">
                <div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center mx-auto mb-3">
                  {item.icon === 'mobile' ? <HiOutlineDeviceMobile className="w-6 h-6 text-green-600" /> :
                    item.icon === 'watch' ? <MdOutlineWatchLater className="w-6 h-6 text-green-600" /> :
