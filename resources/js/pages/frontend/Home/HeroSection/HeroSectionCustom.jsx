...
const TitleSection = ({
  ...
  return (
    <h1 className={`${titleWeight} ${titleLeading} ${titleTracking} ${titleSize} ${titleMaxWidth}`}>
      {title.split(titleHighlight).map((part, index, array) => (
        <React.Fragment key={index}>
          <span className={`${textColor} ${textColorDark}`}>{part}</span>
          {index < array.length - 1 && (
            <span
              className={
                titleHighlightType === 'gradient'
                  ? `text-transparent bg-clip-text bg-linear-to-r ${titleHighlightGradient} ${titleHighlightGradientDark}`
                  : titleHighlightType === 'underline'
                    ? `${titleHighlightUnderline} ${titleHighlightUnderlineDark}`
                    : `${titleHighlightColor} ${titleHighlightColorDark}`
              }
            >
              {titleHighlight}
            </span>
...
// =============================================
// SECTION 4: DESCRIPTION COMPONENT
// =============================================
...
const DescriptionSection = ({
  ...
  return (
    <p
      className={`${descriptionMaxWidth} ${descriptionSize} ${descriptionWeight} ${descriptionLeading} 
