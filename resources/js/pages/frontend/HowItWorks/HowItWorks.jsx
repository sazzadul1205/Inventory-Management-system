...
const PageHero = ({ heroData }) => {
  ...
  const description = heroData?.description || 'Get started with our proven methodology designed to deliver results quickly and efficiently.';
    ...
    >
      ...
      <div className="container mx-auto px-4 py-20 md:py-28 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" className="flex items-center justify-center gap-2 text-sm text-purple-100 dark:text-purple-200 mb-6">
            <a href="/" className="hover:text-white transition-colors">Home</a>
            <span aria-hidden="true">•</span>
            <span className="text-white" aria-current="page">How It Works</span>
          </nav>
          ...
          >
            {stats.map((stat, index) => (
              <div
                key={index}
                className="bg-white/10 dark:bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10"
                role="listitem"
              >
                <div className="text-3xl font-bold text-yellow-300 dark:text-yellow-400 mb-1">{stat.value}</div>
                <div className="text-sm text-purple-100 dark:text-purple-200">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
            <button
              className="px-8 py-3 bg-yellow-400 dark:bg-yellow-500 text-purple-900 font-semibold rounded-lg hover:bg-yellow-300 transition-colors transform hover:scale-105 duration-200"
              aria-label="Start your journey"
            >
              Get Started Today
            </button>
            <button
              className="px-8 py-3 bg-transparent border-2 border-white text-white font-semibold rounded-lg hover:bg-white/10 transition-colors transform hover:scale-105 duration-200"
